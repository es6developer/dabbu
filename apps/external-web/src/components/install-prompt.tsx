...
export function InstallPrompt({ className }: { className?: string }) {
  ...
  const handleInstall = async () => {
    if (deferredPrompt) {
      ...
      if (platform === 'ios') {
        window.open('https://apps.apple.com/app/dabbu-split', '_blank');
      } else if (platform === 'android') {
        window.open('https://play.google.com/store/apps/details?id=app.dabbu.split', '_blank');
      }
    }
  };
  ...
  if (!isInstallable && platform === 'other') {
    return null;
  }
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h4 className="text-sm font-semibold text-dabbu-text">
                {platform === 'ios' ? 'Install Dabbu Split' : 'Add to Home Screen'}
              </h4>
              ...
              {platform === 'ios' && !isInstallable && (
                <p className="text-[10px] text-dabbu-text-muted mt-1">
                  Tap the share button <span className="inline-block">⎙</span> and select &ldquo;Add
