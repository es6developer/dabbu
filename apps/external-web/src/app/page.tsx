...
const FEATURES = [
  {
    icon: '🪙',
    title: 'Smart Splits',
    desc: 'Split expenses equally, by percentage, or exact amounts. Real-time calculations for everyone.',
  },
  {
    icon: '✈️',
    title: 'Trip Friendly',
    desc: "Perfect for group trips. Track who paid for what and settle up effortlessly when you're back.",
  },
  {
    icon: '✅',
    title: 'Easy Settlements',
    desc: 'See exactly who owes what and settle up with a tap. Track paid and pending amounts.',
  },
  {
    icon: '💬',
    title: 'Group Chat',
    desc: 'Discuss expenses and plans with built-in group chat. Messages, payments, and updates in one place.',
  },
];
          ...
          <MobileContainer>
            <View style={s.navRow}>
              <View style={{ flexDirection: 'row', gap: 10, alignItems: 'center' }}>
                ...
                <Text style={[s.brandText, { color: 'var(--dabbu-text)' }]}>
                  Dabbu <Text style={{ color: 'var(--dabbu-accent)' }}>Split</Text>
                </Text>
              </View>
              <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
                <TouchableOpacity
                  onPress={toggleTheme}
                  style={[s.themeBtn, { borderColor: 'var(--dabbu-border)' }]}
                >
            ...
            {isAuthenticated ? (
              <View style={s.heroContent}>
                ...
                >
                  ...
                  <Text style={{ fontSize: 13, color: 'var(--dabbu-text-secondary)' }}>
                    Signed in as {user?.name}
                  </Text>
                </View>
                <Spacer size="lg" />
                <Text style={[s.heroTitle, { color: 'var(--dabbu-text)' }]}>
                  My <Text style={{ color: 'var(--dabbu-accent)' }}>Groups</Text>
                </Text>
                <Spacer size="md" />
                {loadingGroups ? (
                  <View style={s.centerBox}>
                    <View
                      style={[
                        ...
                        {
                          borderColor: 'var(--dabbu-border)',
                          borderTopColor: 'var(--dabbu-accent)',
                        },
                      ]}
                    />
                    <Spacer size="sm" />
                    <Text style={{ fontSize: 13, color: 'var(--dabbu-text-muted)' }}>
                      Loading your groups...
                    </Text>
                  </View>
                ) : myGroups.length === 0 ? (
                  <View style={s.centerBox}>
                    <Text style={[s.emptyTitle, { color: 'var(--dabbu-text-secondary)' }]}>
                      No groups yet
                    </Text>
                    <Spacer size="sm" />
                    <Text style={[s.emptyDesc, { color: 'var(--dabbu-text-muted)' }]}>
                      Join with an invite code or create a new group.
                    </Text>
                  </View>
                ) : (
                  <View style={s.groupList}>
                    {myGroups.map((g) => (
                      ...
                      >
                        ...
                        <View style={{ flex: 1 }}>
                          <Text style={[s.groupName, { color: 'var(--dabbu-text)' }]}>
              ...
              <View style={s.heroContent}>
                ...
                >
                  ...
                  <Text style={{ fontSize: 13, color: 'var(--dabbu-text-muted)' }}>
                    <Text style={{ color: 'var(--dabbu-text-secondary)', fontWeight: '600' }}>
                      2.4k+
                    </Text>{' '}
                    active groups
                  </Text>
        ...
        <View style={s.features}>
          <MobileContainer>
            <View style={{ alignItems: 'center', marginBottom: 40 }}>
              <Text style={[s.sectionTitle, { color: 'var(--dabbu-text)' }]}>
                Everything you need to{' '}
                <Text style={{ color: 'var(--dabbu-accent)' }}>split together</Text>
              </Text>
              <Spacer size="sm" />
              <Text style={[s.sectionSub, { color: 'var(--dabbu-text-secondary)' }]}>
            ...
            <View style={s.featureGrid}>
              {FEATURES.map((feature, i) => (
                <Card key={i} style={s.featureCard}>
                  ...
                  <Spacer size="md" />
                  <Text style={[s.featureTitle, { color: 'var(--dabbu-text)' }]}>
                    {feature.title}
                  </Text>
                  <Spacer size="xs" />
                  <Text style={[s.featureDesc, { color: 'var(--dabbu-text-secondary)' }]}>
                    {feature.desc}
                  </Text>
                </Card>
              ))}
        ...
        <View style={s.cta}>
          <MobileContainer>
            <View
              style={[
                s.ctaCard,
                { backgroundColor: 'var(--dabbu-surface)', borderColor: 'var(--dabbu-border)' },
              ]}
            >
              <Text style={[s.ctaTitle, { color: 'var(--dabbu-text)' }]}>
                Ready to simplify your{' '}
                <Text style={{ color: 'var(--dabbu-accent)' }}>shared finances</Text>?
              </Text>
              <Spacer size="md" />
              <Text style={[s.ctaSub, { color: 'var(--dabbu-text-secondary)' }]}>
...
const s = StyleSheet.create({
  ...
  nav: {
    borderBottomWidth: 1,
    backgroundColor: 'var(--dabbu-nav-bg)',
