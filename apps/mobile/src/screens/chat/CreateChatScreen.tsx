...
export function CreateChatScreen() {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.bg.primary }]} contentContainerStyle={styles.content}>
      ...
      <TextInput style={[styles.input, { backgroundColor: colors.bg.tertiary, color: colors.text.primary, borderColor: colors.border.subtle }]} value={title} onChangeText={setTitle} placeholder="Enter chat name" placeholderTextColor={colors.text.tertiary} />
      ...
      <TouchableOpacity style={[styles.createBtn, { backgroundColor: colors.accent.primary }, saving && { opacity: 0.6 }]} onPress={handleCreate} disabled={saving}>
        {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.createBtnText}>Create Chat</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
}
