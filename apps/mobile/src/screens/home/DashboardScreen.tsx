...
export function DashboardScreen() {
  ...
  const quickActions = [
    { icon: 'camera-outline', label: 'Scan Bill', color: '#FF6B6B', action: () => navigation.navigate('Accounts', { screen: 'BillScanner' }) },
    { icon: 'add-circle-outline', label: 'Add Expense', color: '#00B894', action: () => navigation.navigate('Accounts', { screen: 'AddExpense' }) },
    { icon: 'swap-horizontal-outline', label: 'Transfer', color: '#FDCB6E', action: () => navigation.navigate('Accounts', { screen: 'CreateTransaction' }) },
    { icon: 'pie-chart-outline', label: 'Budgets', color: '#74B9FF', action: () => navigation.navigate('Accounts', { screen: 'ExpenseHome', params: { screen: 'AccountsList' } }) },
  ];
      ...
      >
          ...
          <View style={styles.header}>
            <View style={styles.headerTop}>
              <View style={styles.headerLeft}>
                ...
                <Text style={[styles.name, { color: colors.text.primary }]}>{user?.firstName || 'there'}</Text>
              </View>
              <View style={styles.headerRight}>
                ...
                <TouchableOpacity style={styles.avatarWrap} onPress={() => navigation.navigate('Settings', { screen: 'SettingsMain' })}>
                  <LinearGradient colors={['#f7892c', '#ff9f43']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.avatar}>
                    <Text style={styles.avatarText}>{user?.firstName?.[0] || 'U'}</Text>
                  </LinearGradient>
                  <View style={[styles.avatarDot, { backgroundColor: colors.status.success, borderColor: colors.bg.primary }]} />
                </TouchableOpacity>
              </View>
            </View>
          </View>
          ...
          <Card variant="glass" style={styles.balanceCard} padding="xl">
            <View style={styles.balanceTop}>
              <View style={styles.balanceLabelRow}>
                ...
                <Text style={[styles.balanceLabel, { color: colors.text.tertiary }]}>Total Balance</Text>
              </View>
              <TouchableOpacity style={[styles.eyeBtn, { backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)' }]}>
            ...
            <View style={styles.balanceMeta}>
              <View style={styles.balanceMetaItem}>
                <Text style={[styles.balanceMetaLabel, { color: colors.text.tertiary }]}>Income</Text>
                <Text style={[styles.balanceMetaValue, { color: colors.status.success }]}>+{formatCurrency(monthlyIncome)}</Text>
              </View>
              <View style={[styles.balanceMetaDivider, { backgroundColor: colors.border.subtle }]} />
              <View style={styles.balanceMetaItem}>
                <Text style={[styles.balanceMetaLabel, { color: colors.text.tertiary }]}>Spent</Text>
                <Text style={[styles.balanceMetaValue, { color: colors.status.error }]}>-{formatCurrency(monthlyExpense)}</Text>
              </View>
              <View style={[styles.balanceMetaDivider, { backgroundColor: colors.border.subtle }]} />
              <View style={styles.balanceMetaItem}>
                <Text style={[styles.balanceMetaLabel, { color: colors.text.tertiary }]}>Accounts</Text>
                <Text style={[styles.balanceMetaValue, { color: colors.text.primary }]}>{totalAccounts}</Text>
              </View>
            </View>
          </Card>
        </LinearGradient>
        ...
        {categoryData.length > 0 && (
          <View style={styles.section}>
            <View style={styles.sectionHeader}>
              <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>Spending Breakdown</Text>
            ...
            <View style={styles.breakdownContainer}>
              <View style={styles.ringSection}>
                <Svg width={110} height={110} viewBox="0 0 110 110">
                  <Circle cx="55" cy="55" r={ringRadius} stroke={isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'} strokeWidth="10" fill="none" />
                  {categoryData.slice(0, 4).map((cat: any, i: number) => {
                    const pct = Math.min(cat.percentage || 0, 100);
                    const dashLen = (pct / 100) * ringCirc;
                    const offset = categoryData.slice(0, i).reduce((s: number, c: any) => s - (Math.min(c.percentage || 0, 100) / 100) * ringCirc, -ringCirc * 0.25);
                    return (
                      <Circle
                        key={i}
                ...
                <View style={styles.ringCenter}>
                  <Text style={[styles.ringCenterValue, { color: colors.text.primary }]}>
                    {categoryData.reduce((s: number, c: any) => s + (c.percentage || 0), 0).toFixed(0)}%
                  </Text>
                  <Text style={[styles.ringCenterLabel, { color: colors.text.tertiary }]}>spent</Text>
                </View>
              </View>
              <View style={styles.breakdownList}>
                {categoryData.slice(0, 4).map((cat: any, i: number) => (
                  <View key={i} style={styles.breakdownRow}>
                    <View style={[styles.breakdownDot, { backgroundColor: cat.color || ['#FF6B6B', '#FDCB6E', '#00B894', '#74B9FF'][i] }]} />
                    <Text style={[styles.breakdownName, { color: colors.text.secondary }]} numberOfLines={1}>{cat.name}</Text>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity
              onPress={() => navigation.navigate('Accounts', { screen: 'ExpenseHome', params: { screen: 'TransactionsList' } })}
          ...
          {txn.length === 0 ? (
            <Card variant="glass" style={styles.emptyCard} padding="xl">
              <View style={styles.emptyContent}>
                ...
                <Text style={[styles.emptyDesc, { color: colors.text.tertiary }]}>Tap + to add your first transaction or scan a bill</Text>
              </View>
            </Card>
          ) : (
            txn.map((item: any, i: number) => {
              ...
              return (
                ...
                >
                  <View style={[styles.txnIconWrap, { backgroundColor: isExpense ? colors.status.error + '18' : colors.status.success + '18' }]}>
                    <Ionicons name={isExpense ? 'arrow-up' : 'arrow-down'} size={16} color={isExpense ? colors.status.error : colors.status.success} />
                  </View>
                  <View style={styles.txnInfo}>
                    <Text style={[styles.txnName, { color: colors.text.primary }]} numberOfLines={1}>
                  ...
                  <View style={styles.txnRight}>
                    <Text style={[styles.txnAmount, { color: isExpense ? colors.status.error : colors.status.success }]}>
...
const styles = StyleSheet.create({
  ...
  notifBtn: { width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  ...
  avatarDot: { width: 10, height: 10, borderRadius: 5, position: 'absolute', bottom: -2, right: -2, borderWidth: 2 },
  ...
  balanceMetaLabel: { fontSize: 10, fontWeight: '500', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 2 },
  balanceMetaValue: { fontSize: 15, fontWeight: '700' },
  balanceMetaDivider: { width: 1, height: 32, marginHorizontal: 8 },
  quickActions: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 24, gap: 12, marginTop: 24, marginBottom: 28 },
  ...
  ringSection: { width: 110, height: 110, position: 'relative', justifyContent: 'center', alignItems: 'center' },
  ...
  txnCard: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 16, borderRadius: 18, marginBottom: 8, gap: 14 },
  txnIconWrap: { width: 36, height: 36, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  txnInfo: { flex: 1 },
