...
export function NotificationCenterScreen() {
  ...
  const loadNotifications = async (isRefresh = false) => {
    try {
      ...
      if (activeFilter === 'overdue') url += '&overdue=true';
      else if (activeFilter === 'upcoming') url += '&type=reminder_upcoming';
      else if (activeFilter === 'paid') url += '&type=goal_milestone,completed';
      else if (activeFilter === 'bill' || activeFilter === 'subscription') url += `&category=${activeFilter}`;
  ...
  const handlePress = (item: NotificationItem) => {
    ...
    if (item.data?.reminderId) {
      ...
      navigation.navigate('Shared', { screen: 'GroupDetail', params: { groupId: item.data.groupId } });
    }
  };
    ...
    return (
      ...
      >
        <View style={styles.cardHeader}>
          <View style={[styles.iconContainer, { backgroundColor: item.overdue ? '#FF3B3020' : priorityColor + '20' }]}>
            <Ionicons
          ...
          {unreadCount > 0 && (
            <>
              <TouchableOpacity onPress={handleMarkAllRead} style={styles.markAllBtn}>
                <Text style={[styles.markAllText, { color: colors.accent.primary }]}>Mark all read</Text>
              </TouchableOpacity>
