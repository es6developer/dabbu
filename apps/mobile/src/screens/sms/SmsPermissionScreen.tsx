...
import {
  checkSmsPermission, requestSmsPermission, ensureSmsPermission,
  isSmsModuleAvailable, getAndroidApiLevel, isPermissionRestricted,
  openAppSettings, getAdbGrantCommand, getAndroidPermissionExplanation,
  SmsPermissionStatus,
...
export function SmsPermissionScreen() {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.bg.primary }]} contentContainerStyle={{ ...styles.content, paddingTop: insets.top + 16 }}>
      <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
        ...
        {Platform.OS !== 'android' ? (
          <View style={[styles.infoCard, { backgroundColor: `${colors.status.warning}15`, borderColor: `${colors.status.warning}30` }]}>
          ...
          <View style={styles.centerBox}>
            ...
            <Text style={[styles.statusText, { color: colors.text.tertiary }]}>Checking permission...</Text>
          </View>
        ) : state === 'permission' ? (
          <View style={styles.permissionSection}>
            <View style={[styles.statusCard, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]}>
              <View style={styles.statusRow}>
                <View style={[styles.dot, {
                  backgroundColor: permStatus === 'granted' ? colors.status.success
                    : permStatus === 'restricted' ? colors.status.warning
                    : colors.status.error
                }]} />
                <View style={styles.statusInfo}>
                  <Text style={[styles.statusLabel, { color: colors.text.primary }]}>Permission</Text>
                  <Text style={[styles.statusValue, { color: colors.text.tertiary }]}>
                    {permStatus === 'granted' ? 'Granted'
                    : permStatus === 'restricted' ? 'Restricted'
                    : permStatus === 'never_ask_again' ? 'Permanently Denied'
                    : 'Not Granted'}
                  </Text>
                </View>
                {!moduleAvailable && (
                  <View style={[styles.warnBadge, { backgroundColor: `${colors.status.warning}20` }]}>
                    <Text style={[styles.warnBadgeText, { color: colors.status.warning }]}>No Module</Text>
                  </View>
                )}
              </View>
            </View>
            ...
            <TouchableOpacity
              style={[styles.primaryBtn, { backgroundColor: colors.accent.primary, opacity: moduleAvailable ? 1 : 0.5 }]}
              onPress={handleGrant}
            ...
            {permStatus === 'never_ask_again' && (
              <View style={[styles.infoCard, { backgroundColor: `${colors.status.error}12`, borderColor: `${colors.status.error}25` }]}>
                <Ionicons name="alert-circle" size={18} color={colors.status.error} />
                <Text style={[styles.infoText, { color: colors.text.secondary }]}>
                  Permission was permanently denied. Open system Settings to enable it, or paste SMS manually below.
                </Text>
                <TouchableOpacity style={[styles.smallBtn, { backgroundColor: `${colors.accent.primary}20` }]} onPress={openAppSettings}>
                  <Text style={[styles.smallBtnText, { color: colors.accent.primary }]}>Open Settings</Text>
                </TouchableOpacity>
              </View>
            )}

            {permStatus === 'restricted' && (
              <View style={[styles.infoCard, { backgroundColor: `${colors.status.warning}12`, borderColor: `${colors.status.warning}25` }]}>
          ...
          <View style={styles.centerBox}>
            <ActivityIndicator color={colors.accent.primary} size="large" />
            <Text style={[styles.statusText, { color: colors.text.tertiary }]}>Reading SMS messages...</Text>
          </View>
        ) : state === 'done' ? (
          <View style={styles.doneSection}>
            <View style={[styles.resultCard, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]}>
              <View style={[styles.checkCircle, { backgroundColor: `${colors.status.success}20` }]}>
                <Ionicons name="checkmark-circle" size={40} color={colors.status.success} />
              </View>
              <Text style={[styles.resultTitle, { color: colors.text.primary }]}>Sync Complete</Text>
              <View style={styles.statsRow}>
                <View style={[styles.statBox, { backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)' }]}>
                  <Text style={[styles.statNum, { color: colors.accent.primary }]}>{parsedCount}</Text>
                  <Text style={[styles.statLabel, { color: colors.text.tertiary }]}>Detected</Text>
                </View>
                <View style={[styles.statBox, { backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)' }]}>
                  <Text style={[styles.statNum, { color: colors.status.success }]}>{uploadedCount}</Text>
                  <Text style={[styles.statLabel, { color: colors.text.tertiary }]}>Uploaded</Text>
                </View>
              </View>
              <TouchableOpacity
              ...
              >
                ...
                <Text style={[styles.secondaryBtnText, { color: colors.accent.primary }]}>View Detections</Text>
              </TouchableOpacity>
            </View>
          </View>
        ) : null}
        ...
        <View style={[styles.manualCard, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]}>
          ...
          />
          <TouchableOpacity
            style={[styles.primaryBtn, { backgroundColor: colors.accent.primary, opacity: manualText.trim() ? 1 : 0.5 }]}
          ...
          {manualResult && (
            <View style={[styles.parseResult, { backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)', borderColor: colors.border.subtle }]}>
              <Text style={[styles.parseResultTitle, { color: colors.text.primary }]}>Parsed Result</Text>
              {[
                { label: 'Amount', value: manualResult.amount ? `₹${Number(manualResult.amount).toLocaleString('en-IN')}` : 'N/A', color: colors.text.primary },
                { label: 'Merchant', value: manualResult.merchantName || manualResult.merchant || 'N/A', color: colors.text.primary },
                { label: 'Type', value: manualResult.transactionType || manualResult.type || 'N/A', color: manualResult.transactionType === 'credit' ? colors.status.success : colors.status.error },
                { label: 'Category', value: manualResult.categoryName || manualResult.category || 'N/A', color: colors.accent.primary },
              ].map((r, i) => (
                <View key={i} style={styles.parseRow}>
                  <Text style={[styles.parseLabel, { color: colors.text.tertiary }]}>{r.label}</Text>
                  <Text style={[styles.parseValue, { color: r.color, fontWeight: '600' }]}>{r.value}</Text>
                </View>
              ))}
              {manualResult.confidence != null && (
                <View style={[styles.confidenceRow, { borderTopColor: colors.border.subtle }]}>
                  <Text style={[styles.parseLabel, { color: colors.text.tertiary }]}>Confidence</Text>
                  <View style={[styles.confBadge, { backgroundColor: `${manualResult.confidence >= 0.7 ? colors.status.success : manualResult.confidence >= 0.4 ? colors.status.warning : colors.status.error}20` }]}>
                    <Text style={{
                      color: manualResult.confidence >= 0.7 ? colors.status.success
                        : manualResult.confidence >= 0.4 ? colors.status.warning
                        : colors.status.error,
        ...
        >
          ...
          <Text style={[styles.troubleToggleText, { color: colors.text.tertiary }]}>Troubleshooting</Text>
          <Ionicons name={showTrouble ? 'chevron-up' : 'chevron-down'} size={14} color={colors.text.tertiary} />
        </TouchableOpacity>

        {showTrouble && (
          <View style={[styles.troubleContent, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]}>
            ...
            <TroubleRow label="SMS Module" value={moduleAvailable ? 'Available' : 'Not Available'} colors={colors}
              valueColor={moduleAvailable ? colors.status.success : colors.status.error} />
            <TroubleRow label="Permission" value={permStatus} colors={colors}
              valueColor={permStatus === 'granted' ? colors.status.success : colors.status.error} />
...
function TroubleRow({ label, value, colors, valueColor, mono }: {
  ...
  return (
    <View style={troubleStyles.row}>
      ...
      <Text style={[troubleStyles.value, { color: valueColor || colors.text.secondary }, mono && troubleStyles.mono]}>
        {value}
      </Text>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  smallBtn: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10, alignSelf: 'flex-start' },
  ...
  secondaryBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 13, borderRadius: 14, borderWidth: 1, width: '100%' },
  ...
  confidenceRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, borderTopWidth: 1, marginTop: 6 },
  confBadge: { paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8 },
  troubleToggle: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 14, marginTop: 16, borderRadius: 14, borderWidth: 1 },
  troubleToggleText: { fontSize: 13, fontWeight: '500' },
  troubleContent: { borderRadius: 18, borderWidth: 1, padding: 16, marginTop: 10 },
  troubleNote: { fontSize: 11, lineHeight: 16, marginTop: 8, fontFamily: Platform.OS === 'ios' ? 'Menlo' : 'monospace' },
});
