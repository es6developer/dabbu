...
export function CoupleSettlementsScreen() {
  ...
  const handleSettleUp = useCallback(() => {
    if (!data) return;
    const { balance } = data;
    const owedBy = balance.youOwePartner ? 'you owe' : 'partner owes';
    const amount = Math.abs(balance.netBalance);
    const recipient = balance.youOwePartner ? 'your partner' : 'you';

    Alert.alert(
      'Settle Up',
      `Confirm settlement of ${fmt(amount)}?\n\n${owedBy === 'you owe' ? 'You' : 'Your partner'} will pay ${recipient}.`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert('Settled!', 'Your balance has been settled successfully.');
              setRefreshing(true);
              fetchData(true);
            } catch (err: any) {
        ...
        <View style={{ paddingHorizontal: 20, marginTop: -16, gap: 16 }}>
            ...
            <View style={[styles.balanceCard, { backgroundColor: '#FFEBB4' }]}>
              ...
              <View style={styles.balanceRow}>
                <View style={styles.balanceSide}>
                  ...
                  <Text style={[styles.balanceAmount, { color: bal.youOwePartner ? '#FF4D4F' : '#34C759' }]}>
                    {bal.youOwePartner ? `-${fmt(bal.youOwe)}` : `+${fmt(bal.partnerOwes)}`}
                  </Text>
                  <View style={[styles.balanceTag, { backgroundColor: bal.youOwePartner ? 'rgba(255,77,79,0.15)' : 'rgba(52,199,89,0.15)' }]}>
                    <Text style={[styles.balanceTagText, { color: bal.youOwePartner ? '#FF4D4F' : '#34C759' }]}>
              ...
              <View style={styles.netBalanceRow}>
                ...
                <Text style={[styles.netAmount, { color: bal.netBalance >= 0 ? '#34C759' : '#FF4D4F' }]}>
                  {bal.netBalance >= 0 ? '+' : ''}{fmt(bal.netBalance)}
                </Text>
              </View>
              {bal.youOwePartner ? (
                <TouchableOpacity
          ...
          {data?.outstanding && data.outstanding.length > 0 && (
            <View style={[styles.sectionCard, { backgroundColor: colors.bg.card }]}>
              ...
              {data.outstanding.map((item) => (
                <View key={item.id} style={[styles.outstandingItem, { borderColor: colors.border.subtle }]}>
                  <View style={styles.outstandingLeft}>
                    <Text style={[styles.outstandingDesc, { color: colors.text.primary }]}>{item.description}</Text>
                    <Text style={[styles.outstandingPaidBy, { color: colors.text.tertiary }]}>
                      Paid by {item.paidBy}
                    </Text>
                  </View>
                  <View style={styles.outstandingRight}>
                    <Text style={[styles.outstandingAmt, { color: colors.text.primary }]}>{fmt(item.amount)}</Text>
          ...
          {data?.settlements && data.settlements.length > 0 && (
            <View style={[styles.sectionCard, { backgroundColor: colors.bg.card }]}>
              ...
              {data.settlements.map((item) => (
                <View key={item.id} style={[styles.settlementItem, { borderColor: colors.border.subtle }]}>
                  <View style={styles.settlementTop}>
                    <View style={styles.settlementLeft}>
                      <View style={[styles.statusDot, { backgroundColor: item.status === 'completed' ? '#34C759' : '#F59E0B' }]} />
                      <View>
                        <Text style={[styles.settlementDate, { color: colors.text.tertiary }]}>{fmtDate(item.date)}</Text>
                        <Text style={[styles.settlementParties, { color: colors.text.secondary }]}>
                          {item.from} paid {item.to}
                        </Text>
                      </View>
                    </View>
                    <Text style={[styles.settlementAmt, { color: colors.text.primary }]}>{fmt(item.amount)}</Text>
                  </View>
                  <View style={styles.settlementBottom}>
                    <View style={[styles.statusBadge, { backgroundColor: item.status === 'completed' ? 'rgba(52,199,89,0.12)' : 'rgba(245,158,11,0.12)' }]}>
                      <Text style={[styles.statusBadgeText, { color: item.status === 'completed' ? '#34C759' : '#F59E0B' }]}>
                        {item.status === 'completed' ? 'Completed' : 'Pending'}
                      </Text>
                    </View>
                    <Text style={[styles.methodText, { color: colors.text.tertiary }]}>{item.method}</Text>
                  </View>
                </View>
              ))}
            </View>
          ...
          {error && !data && (
            <View style={[styles.errorCard, { backgroundColor: colors.bg.card }]}>
              <Ionicons name="alert-circle-outline" size={32} color={colors.status?.error || '#FF4D4F'} />
          ...
          {data && !bal && (
            <View style={[styles.errorCard, { backgroundColor: colors.bg.card }]}>
              ...
              <Text style={[styles.errorText, { color: colors.text.secondary }]}>No balance data available</Text>
            </View>
          )}
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  balanceDivider: { width: 1, backgroundColor: 'rgba(93,56,181,0.15)', marginHorizontal: 8, alignSelf: 'stretch' },
  netBalanceRow: {
  ...
  outstandingItem: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingVertical: 12, borderTopWidth: 1, gap: 12,
  },
