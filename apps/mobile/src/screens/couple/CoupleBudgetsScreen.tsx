...
function statusBadge(pct: number): { label: string; bg: string; text: string; icon: string } {
  if (pct > 90) return { label: 'Exceeded', bg: '#FFE5E5', text: '#CC3B3B', icon: 'alert-circle' };
  if (pct > 70) return { label: 'Warning', bg: '#FFF4D9', text: '#B8860B', icon: 'warning' };
      ...
      >
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 12, gap: 12 }}>
          <View style={[styles.heroCard, { backgroundColor: '#FFEBB4' }]}>
            ...
            <View style={[styles.progressBar, { backgroundColor: 'rgba(93,56,181,0.12)', marginTop: 12 }]}>
              <View
                style={[
                  styles.progressFill,
                  {
          ...
          <Text style={[styles.sectionLabel, { color: colors.text.primary }]}>Category Breakdown</Text>
          ...
          {categories.length === 0 && (
            <View style={[styles.emptyCard, { backgroundColor: colors.bg.card }]}>
              ...
              <Text style={[styles.emptyCardText, { color: colors.text.secondary }]}>No categories set up yet</Text>
              <Text style={[styles.emptyCardSub, { color: colors.text.tertiary }]}>Tap "Adjust Budget" to get started</Text>
            </View>
          )}
          ...
          {categories.map((cat: any, i: number) => {
            ...
            return (
              <View key={cat.id || i} style={[styles.catCard, { backgroundColor: colors.bg.card }]}>
                ...
                <View style={[styles.progressBar, { backgroundColor: colors.bg.tertiary, marginTop: 4 }]}>
                  <View
                    style={[
                      styles.progressFill,
                      {
