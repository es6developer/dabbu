...
export function PartnerScreen() {
  ...
  const sinceDate = profile?.relationship?.sinceDate ? new Date(profile.relationship.sinceDate) : null;
        ...
        >
          ...
          <View style={{ paddingHorizontal: 20, marginBottom: 16 }}>
            <LinearGradient colors={[colors.accent.primary, colors.accent.hover]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.profileCard}>
              ...
              <Text style={{ fontSize: 22, fontWeight: '800', color: '#FFF', marginTop: 8 }}>{partner?.name || 'Partner'}</Text>
              {partner?.email && <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)', marginTop: 2 }}>{partner.email}</Text>}
              <View style={styles.statsRow}>
                ...
                <View style={[styles.statDivider, { backgroundColor: 'rgba(255,255,255,0.2)' }]} />
                <View style={styles.statItem}>
                  <Text style={styles.statValue}>{profile?.relationship?.status === 'active' ? 'Connected' : 'Pending'}</Text>
          ...
          <View style={{ paddingHorizontal: 20, marginBottom: 16 }}>
            ...
            {timeline.length === 0 ? (
              <View style={[styles.emptyCard, { backgroundColor: colors.bg.card }]}>
                ...
                <Text style={{ fontSize: 13, color: colors.text.tertiary, marginTop: 6 }}>No timeline events yet</Text>
              </View>
            ) : (
              <View style={{ gap: 8 }}>
                {timeline.map((event: any) => (
                  <View key={event.id} style={[styles.eventCard, { backgroundColor: colors.bg.card }]}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {event.description ? <Text style={{ fontSize: 11, color: colors.text.tertiary, marginTop: 2 }}>{event.description}</Text> : null}
                      <Text style={{ fontSize: 10, color: colors.text.tertiary, marginTop: 4 }}>
                        {event.createdAt ? new Date(event.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : ''}
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { backgroundColor: colors.bg.primary }]}>
            <View style={styles.modalHeader}>
              <Text style={{ fontSize: 18, fontWeight: '700', color: colors.text.primary }}>Add Timeline Event</Text>
              <TouchableOpacity onPress={() => setShowAddEvent(false)}>
                <AntDesign name="close" size={20} color={colors.text.tertiary} />
              </TouchableOpacity>
            </View>
            <Text style={{ fontSize: 13, fontWeight: '600', color: colors.text.secondary, marginBottom: 6, marginTop: 16 }}>Title</Text>
            <TextInput
              style={[styles.input, { backgroundColor: colors.bg.secondary, color: colors.text.primary, borderColor: colors.border.subtle }]}
              value={eventTitle} onChangeText={setEventTitle} placeholder="What happened?" placeholderTextColor={colors.text.tertiary}
            />
            <Text style={{ fontSize: 13, fontWeight: '600', color: colors.text.secondary, marginBottom: 6, marginTop: 12 }}>Description (optional)</Text>
            <TextInput
              style={[styles.input, { backgroundColor: colors.bg.secondary, color: colors.text.primary, borderColor: colors.border.subtle, minHeight: 80 }]}
              value={eventDesc} onChangeText={setEventDesc} placeholder="Add details..." placeholderTextColor={colors.text.tertiary} multiline
            />
            <Text style={{ fontSize: 13, fontWeight: '600', color: colors.text.secondary, marginBottom: 6, marginTop: 12 }}>Type</Text>
...
const styles = StyleSheet.create({
  ...
  partnerAvatar: { width: 72, height: 72, borderRadius: 36, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  ...
  typeChip: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 8, paddingHorizontal: 12, borderRadius: 10 },
  submitBtn: { paddingVertical: 16, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginTop: 8 },
});
