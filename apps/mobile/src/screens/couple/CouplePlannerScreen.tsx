...
import {
  ...
  TextInput, Modal, StyleSheet, Dimensions, Alert,
} from 'react-native';
...
const CATEGORY_COLORS: Record<string, string> = {
  home: '#FF6B6B', travel: '#60A5FA', wedding: '#A78BFA',
  car: '#34C759', baby: '#FF8A65', emergency: '#F59E0B',
  investment: '#14B8A6', education: '#8B5CF6', other: '#64748B',
};

const CATEGORY_ICONS: Record<string, string> = {
  home: 'home', travel: 'airplane', wedding: 'heart',
  car: 'car', baby: 'happy', emergency: 'shield',
  investment: 'trending-up', education: 'school', other: 'flag',
};
...
function PlannerCard({ item, onPress }: { item: any; onPress: () => void }) {
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
      style={{
        width: CARD_WIDTH, backgroundColor: '#161224', borderRadius: 18, padding: 14,
        marginBottom: 10,
      }}
    >
      <View style={{
      ...
      {item.deadline && (
        <Text style={{ fontSize: 10, color: '#475569', marginTop: 4 }}>
          By {new Date(item.deadline).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })}
        </Text>
      )}
    </TouchableOpacity>
  );
...
export function CouplePlannerScreen() {
  ...
  const contribute = useCallback(async (id: string) => {
    Alert.prompt(
      'Add Contribution',
      ...
      async (val) => {
        ...
        try {
          await api.post(`/couple/planners/${id}/contribute`, { amount });
          fetchPlanners(true);
        } catch {}
      },
      'plain-text',
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#0D0B1A' }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
            ...
            <View>
              ...
              <Text style={{ fontSize: 12, color: '#64748B' }}>Dream it, save it, achieve it together</Text>
            </View>
          </View>
          <TouchableOpacity
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <PlannerCard item={item} onPress={() => contribute(item.id)} />
        )}
        ListEmptyComponent={
          <View style={{ padding: 60, alignItems: 'center', gap: 12 }}>
            <View style={{
              width: 64, height: 64, borderRadius: 20, backgroundColor: '#8B5CF618',
      ...
      <Modal visible={showCreate} transparent animationType="slide">
        <View style={{ flex: 1, backgroundColor: '#00000080', justifyContent: 'flex-end' }}>
          <View style={{ backgroundColor: '#161224', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24 }}>
            ...
            <TextInput
              ...
              style={{
                backgroundColor: '#1E293B', borderRadius: 12, padding: 14, fontSize: 14,
              ...
              {Object.keys(CATEGORY_COLORS).map(cat => (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    backgroundColor: form.category === cat ? `${CATEGORY_COLORS[cat]}30` : '#1E293B',
                    borderWidth: 1,
                ...
                >
                  <Text style={{
                    fontSize: 12, fontWeight: '600', color: form.category === cat ? CATEGORY_COLORS[cat] : '#94A3B8',
                  }}>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              <TouchableOpacity
                style={{ flex: 1, padding: 14, borderRadius: 14, backgroundColor: '#1E293B', alignItems: 'center' }}
              ...
              >
                <Text style={{ fontSize: 14, fontWeight: '700', color: form.title && form.targetAmount ? '#FFF' : '#475569' }}>
                  Create
                </Text>
              </TouchableOpacity>
            </View>
