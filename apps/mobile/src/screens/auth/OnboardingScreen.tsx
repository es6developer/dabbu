...
const slides = [
  { icon: 'bulb' as const, title: 'Never Miss a Thing', desc: 'Smart reminders for bills, medicines, and everything you tend to forget.' },
  { icon: 'trending-down' as const, title: 'Track Subscriptions', desc: 'Know exactly where your money goes each month. No more surprise charges.' },
  { icon: 'people' as const, title: 'Family Coordination', desc: 'Share reminders, tasks, and finances with your family seamlessly.' },
];
...
export function OnboardingScreen() {
  ...
  return (
    ...
    >
      <TouchableOpacity style={styles.skip} onPress={() => { markSeen(); navigation.replace('Login'); }}>
      ...
      <View style={[styles.footer, { backgroundColor: colors.bg.primary }]}>
        <View style={styles.dots}>
          {slides.map((_, i) => (
            <View key={i} style={[styles.dot, { backgroundColor: colors.text.tertiary }, i === index && { width: 24, backgroundColor: colors.accent.primary }]} />
          ))}
        </View>
        ...
        {index < slides.length - 1 && (
          <TouchableOpacity style={styles.skipLater} onPress={async () => { await markSeen(); navigation.replace('Login'); }}>
...
const styles = StyleSheet.create({
  ...
  iconWrap: { width: 96, height: 96, borderRadius: 28, alignItems: 'center', justifyContent: 'center', marginBottom: 28 },
  ...
  footer: { paddingHorizontal: 24, paddingTop: 28, paddingBottom: 48, borderTopLeftRadius: 28, borderTopRightRadius: 28 },
