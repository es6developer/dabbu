...
export function ForgotPasswordScreen() {
  ...
  return (
    <LinearGradient colors={isDark ? [colors.bg.secondary, colors.bg.primary] : ['#f8f4f0', colors.bg.primary]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={styles.container}>
      ...
      <Text style={[styles.subtitle, { color: colors.text.tertiary }]}>Enter your email and we'll send you a reset link</Text>
      {error ? (
      ...
      {sent ? (
        <View style={[styles.sentBox, { backgroundColor: colors.status.successLight }]}>
          ...
          <Text style={[styles.sentText, { color: colors.status.success }]}>Check your email for the reset link</Text>
        </View>
      ) : (
        <>
          <TextInput
            style={[styles.input, { backgroundColor: colors.bg.tertiary, color: colors.text.primary, borderColor: colors.border.subtle }]}
            placeholder="Email" placeholderTextColor={colors.text.tertiary}
            value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address"
          />
          <TouchableOpacity style={[styles.button, { backgroundColor: colors.accent.primary }, loading && styles.buttonDisabled]} onPress={handleReset} disabled={loading}>
