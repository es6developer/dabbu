...
import { View, Text, TouchableOpacity, ScrollView, ActivityIndicator, RefreshControl, StyleSheet, Dimensions } from 'react-native';
...
export function FamilyWorkspaceScreen() {
  ...
  const handleQuickAction = (screen: string, params?: any) => {
    navigation.navigate('WalletTab', {
      screen,
      params: { ...params, returnTo: 'FamilyWorkspace' },
    });
  };
  ...
  if (!data?.workspace) {
    return (
      <View style={[s.centered, { backgroundColor: colors.bg.primary, padding: 32, gap: 16 }]}>
        ...
        <Text style={[s.noWorkspaceTitle, { color: colors.text.primary }]}>
          No Workspace Yet
        </Text>
        <TouchableOpacity
  ...
  const hero = dashboard.familyHero || {};
  const snapshot = dashboard.familySnapshot || {};
  const goals = dashboard.familyGoals || [];
  const bills = dashboard.familyBills || {};
  const health = dashboard.familyHealth || {};
  const insights = dashboard.familyInsights || [];
  const recentTransactions = dashboard.recentTransactions || [];
  const members = hero.members || [];
  const memberActivity = dashboard.memberActivity || {};
  const wealth = dashboard.familyWealth || {};
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg.primary }}>
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={() => loadDashboard(true)} tintColor={coverColor} />
        }
      >
        <LinearGradient
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                {workspace.description && (
                  <Text style={{ fontSize: 13, color: colors.text.tertiary, marginTop: 2 }} numberOfLines={2}>
                    {workspace.description}
                  </Text>
                )}
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8 }}>
                  ...
                  <Text style={{ fontSize: 12, color: colors.text.tertiary, marginLeft: 4 }}>
                    {hero.memberCount || members.length} member{(hero.memberCount || members.length) !== 1 ? 's' : ''}
                  </Text>
                </View>
              </View>
            </View>
          ...
          <View style={[s.card, { backgroundColor: colors.bg.card }]}>
            <View style={{ flexDirection: 'row' }}>
              <View style={{ flex: 1, alignItems: 'center', gap: 4 }}>
                ...
                <Text style={[s.value, { color: colors.status.success }]}>{fmtShort(snapshot.income || 0)}</Text>
              </View>
              <View style={[s.vDivider, { backgroundColor: colors.border.subtle }]} />
              <View style={{ flex: 1, alignItems: 'center', gap: 4 }}>
                <Text style={[s.label, { color: colors.text.tertiary }]}>Expenses</Text>
                <Text style={[s.value, { color: colors.status.error }]}>{fmtShort(snapshot.expense || 0)}</Text>
              </View>
              <View style={[s.vDivider, { backgroundColor: colors.border.subtle }]} />
              <View style={{ flex: 1, alignItems: 'center', gap: 4 }}>
                <Text style={[s.label, { color: colors.text.tertiary }]}>Balance</Text>
                <Text style={[s.value, { color: balance >= 0 ? colors.status.success : colors.status.error }]}>
                  {fmtShort(balance)}
                </Text>
              </View>
            </View>
            {snapshot.budgetUtilization > 0 && (
              <View style={{ marginTop: 12, gap: 4 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={{ fontSize: 11, color: colors.text.tertiary }}>Budget Utilization</Text>
                  <Text style={{ fontSize: 11, fontWeight: '600', color: snapshot.budgetUtilization > 80 ? colors.status.error : colors.text.secondary }}>
                ...
                <View style={[s.progressBg, { backgroundColor: colors.bg.tertiary }]}>
                  <View style={[s.progressFill, {
                    ...
                    backgroundColor: snapshot.budgetUtilization > 80 ? colors.status.error : colors.status.success,
                  }]} />
                </View>
              </View>
            )}
        ...
        <View style={{ paddingHorizontal: 20, marginTop: 20, gap: 10 }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
            <QuickActionCard label="Expense" icon="minuscircle" color="#DC2626" onPress={() => handleQuickAction('AddExpense', { type: 'expense' })} colors={colors} />
            <QuickActionCard label="Income" icon="pluscircle" color="#16A34A" onPress={() => handleQuickAction('AddExpense', { type: 'income' })} colors={colors} />
            <QuickActionCard label="Add Bill" icon="filetext1" color="#F59E0B" onPress={() => handleQuickAction('AddBill')} colors={colors} />
            <QuickActionCard label="Family Goal" icon="flag" color="#3B82F6" onPress={() => handleQuickAction('GoalsList')} colors={colors} />
          </View>
        </View>
                ...
                return (
                  <View key={m.id} style={[s.memberCard, { backgroundColor: colors.bg.card }]}>
                    ...
                    <View style={[s.memberRole, { backgroundColor: coverColor + '15' }]}>
                      <Text style={{ fontSize: 9, fontWeight: '700', color: coverColor, textTransform: 'capitalize' }}>
                        {m.role || 'member'}
                      </Text>
                    </View>
                  </View>
            ...
            <View style={{ gap: 10 }}>
              {activeGoals.slice(0, 3).map((g: any) => (
                <View key={g.id} style={[s.card, { backgroundColor: colors.bg.card }]}>
                  ...
                  <View style={[s.progressBg, { backgroundColor: colors.bg.tertiary, marginTop: 6 }]}>
                    <View style={[s.progressFill, {
                      width: `${Math.min(g.progress, 100)}%`,
                      backgroundColor: g.progress >= 80 ? colors.status.success : colors.accent.primary,
                    }]} />
                  </View>
          ...
          <View style={{ paddingHorizontal: 20, marginTop: 20, gap: 10 }}>
            ...
            <View style={{ gap: 8 }}>
              {upcomingBills.map((b: any) => {
                ...
                return (
                  <View key={b.id} style={[s.card, { backgroundColor: colors.bg.card, flexDirection: 'row', alignItems: 'center', gap: 12 }]}>
                    <View style={[s.txnIcon, { backgroundColor: (isUrgent ? colors.status.error : colors.status.warning) + '15' }]}>
                      <AntDesign name="filetext1" size={16} color={isUrgent ? colors.status.error : colors.status.warning} />
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={{ fontSize: 13, fontWeight: '600', color: colors.text.primary }} numberOfLines={1}>
                        {b.name}
            ...
            <View style={{ gap: 8 }}>
              {recentTransactions.slice(0, 10).map((txn: any) => (
                <View key={txn.id} style={[s.txnRow, { backgroundColor: colors.bg.card }]}>
                  <View style={[s.txnIcon, { backgroundColor: (txn.type === 'income' ? colors.status.success : colors.status.error) + '15' }]}>
                    <AntDesign
                  ...
                  <View style={{ flex: 1, gap: 2 }}>
                    <Text style={{ fontSize: 13, fontWeight: '600', color: colors.text.primary }} numberOfLines={1}>
                      {txn.description || txn.category || (txn.type === 'income' ? 'Income' : 'Expense')}
                    </Text>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                      {txn.userName && (
                        <Text style={{ fontSize: 10, color: colors.accent.primary }}>{txn.userName}</Text>
                      )}
                  ...
                  <Text style={{
                    ...
                    {txn.type === 'income' ? '+' : '-'}{fmtShort(Number(txn.amount))}
                  </Text>
                </View>
              ))}
            </View>
        ...
        {health.overallScore > 0 && (
          <View style={{ paddingHorizontal: 20, marginTop: 20, gap: 10 }}>
            ...
            <View style={[s.card, { backgroundColor: colors.bg.card, flexDirection: 'row', alignItems: 'center', gap: 16 }]}>
              <View style={[s.healthRing, { borderColor: health.overallScore >= 70 ? colors.status.success : health.overallScore >= 40 ? colors.status.warning : colors.status.error }]}>
              ...
              <View style={{ flex: 1, gap: 4 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  ...
                  <Text style={{ fontSize: 11, fontWeight: '600', color: colors.text.secondary }}>{health.categoryScores?.savings || 0}%</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={{ fontSize: 11, color: colors.text.tertiary }}>Debt</Text>
                  <Text style={{ fontSize: 11, fontWeight: '600', color: colors.text.secondary }}>{health.categoryScores?.debt || 0}%</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={{ fontSize: 11, color: colors.text.tertiary }}>Goals</Text>
                  <Text style={{ fontSize: 11, fontWeight: '600', color: colors.text.secondary }}>{health.categoryScores?.goals || 0}%</Text>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                  <Text style={{ fontSize: 11, color: colors.text.tertiary }}>Emergency Fund</Text>
                  <Text style={{ fontSize: 11, fontWeight: '600', color: colors.text.secondary }}>{health.categoryScores?.emergencyFund || 0}%</Text>
                </View>
              </View>
            </View>
          </View>
          ...
          <View style={{ paddingHorizontal: 20, marginTop: 20, gap: 10 }}>
            ...
            <View style={{ gap: 8 }}>
              {insights.slice(0, 3).map((insight: string, i: number) => (
                <View key={i} style={[s.insightCard, { backgroundColor: colors.accent.primary + '08', borderColor: colors.accent.primary + '20' }]}>
                  <AntDesign name="bulb1" size={14} color={colors.accent.primary} />
                  <Text style={{ flex: 1, fontSize: 12, color: colors.text.secondary, lineHeight: 18 }}>
                    {insight}
                  </Text>
                </View>
              ))}
...
function QuickActionCard({ label, icon, color, onPress, colors }: any) {
  return (
    <TouchableOpacity onPress={onPress} style={[s.actionCard, { backgroundColor: colors.bg.card }]} activeOpacity={0.7}>
...
const s = StyleSheet.create({
  ...
  createBtn: { paddingVertical: 14, paddingHorizontal: 28, borderRadius: 16, flexDirection: 'row', gap: 8, alignItems: 'center' },
  ...
  actionCard: { width: (width - 50) / 2, borderRadius: 18, padding: 16, alignItems: 'center', gap: 8 },
  ...
  avatarOverflow: { width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  healthRing: { width: 64, height: 64, borderRadius: 32, borderWidth: 4, alignItems: 'center', justifyContent: 'center' },
  insightCard: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, padding: 14, borderRadius: 14, borderWidth: 1 },
});
