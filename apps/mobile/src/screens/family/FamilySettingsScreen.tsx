...
export function FamilySettingsScreen() {
  ...
  function getRoleBadge(role: string) {
    ...
    if (r === 'owner') return { bg: `${colors.accent.primary}26`, color: colors.accent.primary };
    if (r === 'admin') return { bg: `${colors.status.success}26`, color: colors.status.success };
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.bg.primary }]} contentContainerStyle={styles.content}>
      ...
      <View style={[styles.card, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]}>
        ...
        {error ? <View style={[styles.errorBox, { backgroundColor: `${colors.status.error}18` }]}><Text style={[styles.errorBoxText, { color: colors.status.error }]}>{error}</Text></View> : null}
        <TextInput style={[styles.input, { backgroundColor: colors.bg.tertiary, color: colors.text.primary, borderColor: colors.border.subtle }]} value={familyName} onChangeText={setFamilyName} placeholder="Family name" placeholderTextColor={colors.text.tertiary} />
        <TouchableOpacity style={[styles.saveBtn, { backgroundColor: colors.accent.primary }, saving && { opacity: 0.6 }]} onPress={handleSaveName} disabled={saving}>
          {saving ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.saveBtnText}>Save</Text>}
        </TouchableOpacity>
      </View>

      <View style={[styles.card, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]}>
        <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>Members ({members.length})</Text>
        {members.map((member) => {
          const memberId = member.userId || member.id;
          const memberName = member.firstName || member.name || member.user?.firstName || 'Unknown';
          const role = member.role || member.membershipRole || 'member';
          const badgeStyle = getRoleBadge(role);
          const isSelf = memberId === user?.id;
          return (
            <View key={memberId} style={[styles.memberRow, { borderBottomColor: colors.border.subtle }]}>
              ...
              <View style={styles.memberInfo}>
                <Text style={[styles.memberName, { color: colors.text.primary }]}>{memberName} {isSelf ? '(You)' : ''}</Text>
                <View style={[styles.roleBadge, { backgroundColor: badgeStyle.bg }]}>
                  <Text style={[styles.roleText, { color: badgeStyle.color }]}>{role}</Text>
                </View>
              </View>
              {isOwner && !isSelf && (
                <TouchableOpacity style={[styles.removeBtn, { backgroundColor: `${colors.status.error}18` }]} onPress={() => handleRemoveMember(memberId, memberName)}>
                  <Text style={[styles.removeBtnText, { color: colors.status.error }]}>Remove</Text>
                </TouchableOpacity>
              )}
            </View>
          );
      ...
      <View style={styles.actionsSection}>
        {!isOwner && (
          <TouchableOpacity style={[styles.leaveBtn, { backgroundColor: `${colors.status.error}18`, borderColor: `${colors.status.error}40` }]} onPress={handleLeaveFamily}>
            <Text style={[styles.leaveBtnText, { color: colors.status.error }]}>Leave Family</Text>
          </TouchableOpacity>
        )}
        {isOwner && (
          <TouchableOpacity style={[styles.deleteFamilyBtn, { backgroundColor: colors.status.error }]} onPress={handleDeleteFamily}>
