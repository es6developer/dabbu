...
export function ReportsScreen() {
  ...
  const { data: stats, loading, refreshing, refresh } = useApiGet<any>('/transactions/stats?months=6');

  const monthlyData = useMemo(() => {
    if (!stats?.monthlyTrend) return [] as { label: string; amount: number }[];
  ...
  const categoryData = useMemo(() => {
    if (!stats?.categoryBreakdown) return [] as { name: string; amount: number; color: string; pct: number }[];
    const total = (stats.categoryBreakdown as any[]).reduce((s: number, c: any) => s + (c.amount || 0), 0) || 1;
        ...
        <View style={{ paddingHorizontal: 20, paddingTop: 12, gap: 12 }}>
          ...
          {monthlyData.length > 0 && (
            <View style={[styles.sectionCard, { backgroundColor: colors.bg.card }]}>
              <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>Monthly Trend</Text>
          ...
          {categoryData.length > 0 && (
            <View style={[styles.sectionCard, { backgroundColor: colors.bg.card }]}>
              ...
              {categoryData.map((cat: { name: string; amount: number; color: string; pct: number }, i: number) => (
...
const styles = StyleSheet.create({
  ...
  chartRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', height: 140, paddingTop: 8 },
