...
import { useAiColors, AnimatedProgressRing, AiCard, SectionHeader } from './components/AiShared';
...
export function SmartGoalCoachScreen() {
        ...
        await Promise.allSettled(
          list.slice(0, 5).map(async (g: any) => {
            ...
            try {
              ...
              if (p) {
                pred = {
                  predictedCompletionDate: p.predictedCompletionDate,
                  successProbability: p.successProbability,
                  currentPace: p.currentPace,
                  requiredMonthlyContribution: p.requiredMonthlyContribution,
                  improvementTip: p.improvementTip,
                };
              }
            } catch {
              /* ignore */
  ...
  const hero = goals[0];
  const heroPct =
    hero.target > 0 ? Math.min(100, Math.round((hero.current / hero.target) * 100)) : 0;
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={s.heroAmt}>
                  ₹{hero.current.toLocaleString('en-IN')} / ₹{hero.target.toLocaleString('en-IN')}
                </Text>
                <View style={[s.pBar, { backgroundColor: AI_COLORS.border }]}>
                  <View
                    style={[
                      ...
                      {
                        width: `${heroPct}%`,
                        backgroundColor: heroPct >= 80 ? AI_COLORS.success : AI_COLORS.primary,
                      },
                    ]}
                  />
                </View>
              ...
              <AnimatedProgressRing
                ...
                strokeWidth={5}
                progress={heroPct}
                color={heroPct >= 80 ? AI_COLORS.success : AI_COLORS.primary}
              >
                ...
                {hero.predictedCompletionDate && (
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={s.predValue}>
                      {new Date(hero.predictedCompletionDate).toLocaleDateString('en-IN', {
                        day: 'numeric',
                        month: 'short',
                        year: 'numeric',
                      })}
                  ...
                  <View style={{ alignItems: 'flex-end' }}>
                    ...
                    <Text
                      style={[
                        ...
                        {
                          color:
                            hero.successProbability >= 80
                              ? AI_COLORS.success
                              : hero.successProbability >= 50
                                ? AI_COLORS.warning
                                : AI_COLORS.danger,
                        },
                      ]}
            ...
            {hero.improvementTip && (
              <View
                style={[
                  s.tipCard,
                  {
                    backgroundColor: `${AI_COLORS.primary}15`,
                    borderColor: `${AI_COLORS.primary}30`,
                  },
                ]}
              >
                    ...
                    <AiCard padding={16}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          <Text
                            style={{ fontSize: 12, color: AI_COLORS.textSecondary, marginTop: 2 }}
                          >
                            ₹{goal.current.toLocaleString('en-IN')} / ₹
                            {goal.target.toLocaleString('en-IN')}
                          </Text>
                          <View
                            style={[s.pBar, { backgroundColor: AI_COLORS.border, marginTop: 6 }]}
                          >
                            <View
                              style={[
                                s.pBarFill,
                                {
                                  width: `${pct}%`,
                                  backgroundColor:
                                    pct >= 80 ? AI_COLORS.success : AI_COLORS.primary,
                                },
                              ]}
                            />
                          </View>
                        ...
                        <Text
                          style={{
                            fontSize: 22,
                            fontWeight: '800',
                            color: pct >= 80 ? AI_COLORS.success : AI_COLORS.primary,
                          }}
                        >
                          {pct}%
                        </Text>
