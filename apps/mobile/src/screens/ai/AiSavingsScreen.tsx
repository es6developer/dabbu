...
import { AI_COLORS, AiCard, SectionHeader } from './components/AiShared';
    ...
    return (
      <View style={[s.screen, { backgroundColor: AI_COLORS.bg, paddingTop: insets.top + 60 }]}>
        <View style={{ alignItems: 'center', paddingTop: 60, gap: 12 }}>
          ...
          <Text style={{ fontSize: 18, fontWeight: '700', color: AI_COLORS.text }}>No savings opportunities found</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <ReAnimated.View entering={FadeInUp.duration(500)} style={{ paddingHorizontal: 16, marginBottom: 8 }}>
          <AiCard padding={20}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
              <View style={{ width: 64, height: 64, borderRadius: 20, backgroundColor: `${AI_COLORS.success}20`, justifyContent: 'center', alignItems: 'center' }}>
                <Ionicons name="cash-outline" size={30} color={AI_COLORS.success} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 13, color: AI_COLORS.textSecondary }}>Total Potential Savings</Text>
                <Text style={{ fontSize: 32, fontWeight: '800', color: AI_COLORS.text, letterSpacing: -1 }}>₹{totalSavings.toLocaleString('en-IN')}</Text>
                <Text style={{ fontSize: 12, color: AI_COLORS.textSecondary, marginTop: 2 }}>per month · {opportunities.length} opportunity{opportunities.length !== 1 ? 'ies' : 'y'} found</Text>
              </View>
            </View>
          </AiCard>
        </ReAnimated.View>
        ...
        <View style={{ paddingHorizontal: 16, gap: 10 }}>
          {opportunities.map((opp, i) => {
            const typeColor = opp.type.includes('duplicate') || opp.type.includes('unused') ? AI_COLORS.warning
              : opp.type.includes('food') || opp.type.includes('delivery') ? AI_COLORS.success
              : AI_COLORS.info;
            return (
              <ReAnimated.View key={i} entering={FadeInLeft.duration(400).delay(i * 80)}>
                <TouchableOpacity activeOpacity={0.8}>
                  <View style={[s.oppCard, { borderColor: AI_COLORS.border, borderLeftColor: typeColor }]}>
                    <View style={{ flexDirection: 'row', gap: 12 }}>
                      <View style={[s.oppIcon, { backgroundColor: `${typeColor}20` }]}>
                        <Ionicons name={opp.type.includes('subscription') ? 'card-outline' : opp.type.includes('food') ? 'fast-food-outline' : 'cash-outline'} size={22} color={typeColor} />
                      </View>
                      <View style={{ flex: 1 }}>
                        ...
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 8 }}>
                          <View style={[s.oppBadge, { backgroundColor: `${AI_COLORS.success}20` }]}>
                            <Text style={[s.oppBadgeText, { color: AI_COLORS.success }]}>+₹{opp.monthlySavings}/mo</Text>
                          </View>
...
const s = StyleSheet.create({
  ...
  oppCard: { backgroundColor: AI_COLORS.card, borderRadius: 18, padding: 16, borderWidth: 1, borderLeftWidth: 3 },
  oppIcon: { width: 44, height: 44, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
