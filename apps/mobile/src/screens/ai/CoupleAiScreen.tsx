...
import {
  AI_COLORS, AnimatedProgressRing, AiCard, SectionHeader, PremiumBadge, MetricRow,
...
export function CoupleAiScreen() {
    ...
    return (
      <View style={[s.screen, { backgroundColor: AI_COLORS.bg, paddingTop: insets.top + 60 }]}>
        <View style={{ alignItems: 'center', paddingTop: 60, gap: 12 }}>
          ...
          <Text style={{ fontSize: 18, fontWeight: '700', color: AI_COLORS.text }}>No couple data yet</Text>
  ...
  const metrics = [
    { label: 'Budget Discipline', value: data.components?.budgetDiscipline ?? 0, color: AI_COLORS.success },
    ...
    { label: 'Bill Consistency', value: data.components?.billConsistency ?? 0, color: AI_COLORS.purple },
  ];
  ...
  return (
    <View style={[s.screen, { backgroundColor: AI_COLORS.bg }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        <ReAnimated.View entering={FadeInUp.duration(500)} style={{ paddingHorizontal: 16, alignItems: 'center', paddingVertical: 12 }}>
          <AnimatedProgressRing size={140} strokeWidth={10} progress={data.overallScore} color={AI_COLORS.success}>
          ...
          <Text style={s.scoreDesc}>Based on your shared financial health score. Level: {data.financialLevel}</Text>
        </ReAnimated.View>
            ...
            <ReAnimated.View key={i} entering={FadeInUp.duration(300).delay(i * 60)}>
              <AiCard padding={14}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                  <AnimatedProgressRing size={48} strokeWidth={4} progress={m.value} color={m.color}>
                    <Text style={{ fontSize: 12, fontWeight: '800', color: m.color }}>{m.value}</Text>
                  </AnimatedProgressRing>
                  <Text style={{ flex: 1, fontSize: 14, fontWeight: '600', color: AI_COLORS.text }}>{m.label}</Text>
                  <Ionicons name={m.value >= 60 ? 'checkmark-circle' : 'alert-circle'} size={20} color={m.color} />
                </View>
              </AiCard>
            </ReAnimated.View>
          ))}
