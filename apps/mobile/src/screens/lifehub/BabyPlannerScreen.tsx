  ...
  return (
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg.primary }}>
      <View style={{ paddingTop: insets.top + 12, paddingHorizontal: 24, paddingBottom: 8, flexDirection: 'row', alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: 19, fontWeight: '700', color: colors.text.primary, marginLeft: 8 }}>Baby Planner</Text>
      </View>
      <ScrollView contentContainerStyle={{ padding: 24 }} showsVerticalScrollIndicator={false}>
        <View style={{ backgroundColor: colors.bg.card, borderRadius: 28, padding: 24, marginBottom: 20 }}>
          <Text style={{ fontSize: 16, fontWeight: '600', color: colors.text.tertiary, marginBottom: 6 }}>Monthly Income</Text>
          <TextInput
        ...
        {result ? (
          <View style={{ backgroundColor: colors.bg.card, borderRadius: 28, padding: 24 }}>
            ...
            <View style={{ marginBottom: 14 }}>
              ...
              <Text style={{ fontSize: 26, fontWeight: '800', color: colors.text.primary }}>₹{Math.round(result.estimatedMonthlyCost || 0).toLocaleString('en-IN')}</Text>
            </View>
            <View style={{ marginBottom: 14 }}>
              <Text style={{ fontSize: 12, color: colors.text.tertiary }}>Recommended Savings Target</Text>
              <Text style={{ fontSize: 26, fontWeight: '800', color: colors.text.primary }}>₹{Math.round(result.recommendedSavingsTarget || 0).toLocaleString('en-IN')}</Text>
            </View>
            <View>
              <Text style={{ fontSize: 12, color: colors.text.tertiary }}>Timeline</Text>
              <Text style={{ fontSize: 26, fontWeight: '800', color: colors.text.primary }}>{result.timeline ?? '-'}</Text>
            </View>
          </View>
        ) : null}
      </ScrollView>
