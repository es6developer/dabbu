  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg.primary }}>
      <View style={{ paddingTop: insets.top + 12, paddingHorizontal: 20, paddingBottom: 8, flexDirection: 'row', alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: 18, fontWeight: '700', color: colors.text.primary, marginLeft: 8 }}>Retirement Planner</Text>
      </View>
      <ScrollView contentContainerStyle={{ padding: 20 }} showsVerticalScrollIndicator={false}>
        <View style={{ backgroundColor: colors.bg.card, borderRadius: 20, padding: 20, marginBottom: 16 }}>
          <Text style={{ fontSize: 13, fontWeight: '600', color: colors.text.tertiary, marginBottom: 6 }}>Age</Text>
          <TextInput
          ...
          />
          <Text style={{ fontSize: 13, fontWeight: '600', color: colors.text.tertiary, marginBottom: 6, marginTop: 16 }}>Monthly Expense</Text>
          <TextInput
            style={{ backgroundColor: colors.bg.tertiary, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, fontSize: 16, fontWeight: '600', color: colors.text.primary }}
        ...
        {result ? (
          <View style={{ backgroundColor: colors.bg.card, borderRadius: 20, padding: 20 }}>
            ...
            <View style={{ marginBottom: 12 }}>
              ...
              <Text style={{ fontSize: 22, fontWeight: '800', color: colors.text.primary }}>₹{Math.round(result.retirementTarget || 0).toLocaleString('en-IN')}</Text>
            </View>
            <View style={{ marginBottom: 12 }}>
              <Text style={{ fontSize: 12, color: colors.text.tertiary }}>Monthly Investment Needed</Text>
              <Text style={{ fontSize: 22, fontWeight: '800', color: colors.text.primary }}>₹{Math.round(result.monthlyInvestmentNeeded || 0).toLocaleString('en-IN')}</Text>
            </View>
            <View>
              <Text style={{ fontSize: 12, color: colors.text.tertiary }}>Projection</Text>
              <Text style={{ fontSize: 22, fontWeight: '800', color: colors.text.primary }}>{result.projection ?? '-'}</Text>
            </View>
          </View>
        ) : null}
      </ScrollView>
