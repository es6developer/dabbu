...
export function PersonalLensDashboard() {
  ...
  if (loading) {
    return (
      <View style={[styles.screen, { backgroundColor: colors.bg.primary }]}>
        <LinearGradient colors={isDark ? ['#1A0A2E', colors.bg.primary] : ['#F0E6FF', colors.bg.primary]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} locations={[0, 0.3]} style={{ flex: 1, paddingTop: insets.top + 12, paddingHorizontal: 20 }}>
  ...
  return (
    <View style={styles.screen}>
      <LinearGradient colors={isDark ? ['#1A0A2E', colors.bg.primary] : ['#F0E6FF', colors.bg.primary]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} locations={[0, 0.3]} style={{ flex: 1 }}>
        ...
        >
          ...
          <View style={{ paddingHorizontal: 20, marginBottom: 16 }}>
            <View style={[styles.balanceCard, { backgroundColor: '#7C3AED' }]}>
              <Text style={{ fontSize: 13, fontWeight: '600', color: 'rgba(255,255,255,0.7)' }}>Monthly Balance</Text>
              <Text style={{ fontSize: 32, fontWeight: '800', color: '#FFF', marginTop: 4 }}>{fmt(income - expense)}</Text>
              <View style={{ flexDirection: 'row', marginTop: 16, gap: 12 }}>
                <View style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: 12 }}>
                  ...
                  <Text style={{ fontSize: 18, fontWeight: '700', color: '#FFF', marginTop: 2 }}>{fmt(income)}</Text>
                </View>
                <View style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: 12 }}>
                  <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)' }}>Expense</Text>
                  <Text style={{ fontSize: 18, fontWeight: '700', color: '#FFF', marginTop: 2 }}>{fmt(expense)}</Text>
                </View>
                <View style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: 12 }}>
                  <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)' }}>Savings</Text>
                  <Text style={{ fontSize: 18, fontWeight: '700', color: '#FFF', marginTop: 2 }}>{fmt(savings)}</Text>
                </View>
              </View>
              {income > 0 && (
                <View style={{ marginTop: 12, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 8, padding: 10 }}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
                    <Text style={{ fontSize: 11, color: 'rgba(255,255,255,0.7)' }}>Savings Rate</Text>
                  ...
                  <View style={{ height: 4, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 2 }}>
                    <View style={{ width: `${Math.min(savingsRate, 100)}%`, height: 4, backgroundColor: '#FFF', borderRadius: 2 }} />
                  </View>
                </View>
              )}
            </View>
          ...
          <View style={{ paddingHorizontal: 20, marginBottom: 16 }}>
            <Text style={{ fontSize: 15, fontWeight: '700', color: colors.text.primary, marginBottom: 10 }}>Account Balances</Text>
            <View style={[styles.sectionCard, { backgroundColor: colors.bg.card }]}>
              {(d.accountBalances?.length > 0 ? d.accountBalances : []).map((acct: any, i: number) => (
                <TouchableOpacity key={acct.id || i} activeOpacity={0.7} style={[styles.row, i < (d.accountBalances?.length || 0) - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border.subtle }]}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 14, fontWeight: '600', color: colors.text.primary }}>{acct.name}</Text>
                    <Text style={{ fontSize: 11, color: colors.text.tertiary }}>{acct.institution || acct.type}</Text>
                  </View>
                  <Text style={{ fontSize: 16, fontWeight: '700', color: acct.balance >= 0 ? colors.text.primary : '#EF4444' }}>{fmt(acct.balance)}</Text>
                </TouchableOpacity>
              ))}
              {(!d.accountBalances || d.accountBalances.length === 0) && (
                <Text style={{ fontSize: 13, color: colors.text.tertiary, textAlign: 'center', paddingVertical: 20 }}>No accounts linked yet</Text>
              )}
            </View>
          </View>
          ...
          <View style={{ paddingHorizontal: 20, marginBottom: 16 }}>
            <View style={styles.sectionHeader}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('HomeTab', { screen: 'Budgets' })}>
                ...
                <View key={b.id || i} style={[styles.budgetRow, { backgroundColor: colors.bg.card }]}>
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      ...
                      <Text style={{ fontSize: 12, fontWeight: '600', color: b.progress > 80 ? '#EF4444' : colors.text.secondary }}>{fmt(b.spent)} / {fmt(b.limit)}</Text>
                    </View>
                    <View style={{ height: 4, backgroundColor: colors.border.subtle, borderRadius: 2, marginTop: 6 }}>
                      <View style={{ width: `${Math.min(b.progress || 0, 100)}%`, height: 4, backgroundColor: b.progress > 80 ? '#EF4444' : b.progress > 60 ? '#F59E0B' : '#22C55E', borderRadius: 2 }} />
                    </View>
                  </View>
                </View>
              ))}
            ...
            <View style={{ gap: 8 }}>
              {(d.bills?.length > 0 ? d.bills : []).slice(0, 5).map((b: any, i: number) => (
                <View key={b.id || i} style={[styles.billRow, { backgroundColor: colors.bg.card }]}>
                  <View style={[styles.billDot, { backgroundColor: b.paid ? '#22C55E' : '#F59E0B' }]} />
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 13, fontWeight: '600', color: colors.text.primary }}>{b.title}</Text>
                    <Text style={{ fontSize: 11, color: colors.text.tertiary }}>{b.category} · Due {b.dueDate ? new Date(b.dueDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : 'Soon'}</Text>
                  </View>
              ...
              {(!d.bills || d.bills.length === 0) && (
                <View style={[styles.emptyCard, { backgroundColor: colors.bg.card }]}>
                  ...
                  <Text style={{ fontSize: 13, color: colors.text.tertiary, marginTop: 6 }}>No upcoming bills</Text>
                </View>
              )}
            </View>
          </View>
            ...
            <View style={{ gap: 8 }}>
                ...
                <View key={g.id || i} style={[styles.goalRow, { backgroundColor: colors.bg.card }]}>
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      ...
                      <Text style={{ fontSize: 12, fontWeight: '600', color: colors.text.secondary }}>{fmt(g.currentAmount)} / {fmt(g.targetAmount)}</Text>
                    </View>
                    <View style={{ height: 4, backgroundColor: colors.border.subtle, borderRadius: 2, marginTop: 6 }}>
                      <View style={{ width: `${Math.min(g.progress || 0, 100)}%`, height: 4, backgroundColor: g.progress >= 100 ? '#22C55E' : g.progress >= 50 ? '#F59E0B' : '#7C3AED', borderRadius: 2 }} />
                    </View>
                    <Text style={{ fontSize: 10, fontWeight: '500', color: colors.text.tertiary, marginTop: 2 }}>{g.progress}% complete</Text>
                  </View>
                </View>
              ))}
              {(!d.goals || d.goals.length === 0) && (
                <View style={[styles.emptyCard, { backgroundColor: colors.bg.card }]}>
                  ...
                  <Text style={{ fontSize: 13, color: colors.text.tertiary, marginTop: 6 }}>No goals yet</Text>
                </View>
              )}
            </View>
          </View>
          ...
          <View style={{ paddingHorizontal: 20, marginBottom: 16 }}>
            <TouchableOpacity onPress={() => navigation.navigate('HomeTab', { screen: 'NetWorth' })} activeOpacity={0.8}>
              <View style={[styles.netWorthCard, { backgroundColor: colors.bg.card }]}>
                ...
                <Text style={{ fontSize: 28, fontWeight: '800', color: netWorth >= 0 ? '#22C55E' : '#EF4444', marginTop: 4 }}>{fmt(netWorth)}</Text>
          ...
          <View style={{ paddingHorizontal: 20, marginBottom: 16 }}>
            <Text style={{ fontSize: 15, fontWeight: '700', color: colors.text.primary, marginBottom: 12 }}>Quick Actions</Text>
            <View style={styles.qaGrid}>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('WalletTab', { screen: 'AddExpense', params: { type: 'income' } })} style={[styles.qaCard, { backgroundColor: colors.bg.card }]} activeOpacity={0.7}>
                <View style={[styles.qaIcon, { backgroundColor: '#22C55E15' }]}><AntDesign name="pluscircle" size={22} color="#22C55E" /></View>
              ...
              <TouchableOpacity onPress={() => navigation.navigate('HomeTab', { screen: 'Budget' })} style={[styles.qaCard, { backgroundColor: colors.bg.card }]} activeOpacity={0.7}>
                <View style={[styles.qaIcon, { backgroundColor: '#3B82F615' }]}><AntDesign name="wallet" size={22} color="#3B82F6" /></View>
                <Text style={[styles.qaLabel, { color: colors.text.primary }]}>Create Budget</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => navigation.navigate('HomeTab', { screen: 'BillsList' })} style={[styles.qaCard, { backgroundColor: colors.bg.card }]} activeOpacity={0.7}>
                <View style={[styles.qaIcon, { backgroundColor: '#7C3AED15' }]}><AntDesign name="filetext1" size={22} color="#7C3AED" /></View>
