      ...
      >
        ...
        >
          <View style={styles.partnerRow}>
            ...
            <View style={styles.partnerCard}>
              <LinearGradient
                colors={[colors.status.info, '#a29bfe']}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
                style={styles.partnerAvatar}
              >
        ...
        <Card variant="glass" style={styles.sectionCard} padding="xl">
          ...
          <View style={styles.jointRow}>
            <View style={styles.jointItem}>
              <Text style={[styles.jointLabel, { color: colors.text.tertiary }]}>
                Monthly Spending
              </Text>
              <Text style={[styles.jointValue, { color: colors.status.error }]}>
            ...
            <View style={styles.jointItem}>
              ...
              <Text style={[styles.jointValue, { color: colors.status.success }]}>
          ...
          <View style={styles.savingsSection}>
            <View style={styles.savingsRing}>
              ...
              <View style={styles.savingsRingCenter}>
                <Text style={[styles.savingsPct, { color: colors.text.primary }]}>
                  {savingsPct}%
                </Text>
            ...
            <View style={styles.savingsInfo}>
              <Text style={[styles.savingsTitle, { color: colors.text.secondary }]}>
                Shared Savings
              </Text>
              <Text style={[styles.savingsAmount, { color: colors.text.primary }]}>
                {formatCurrency(d.sharedSavingsCurrent || 0)}
              </Text>
              <Text style={[styles.savingsTarget, { color: colors.text.tertiary }]}>
                of {formatCurrency(savingsTarget)} target
              </Text>
            </View>
          </View>
        </Card>
        ...
        <Card variant="glass" style={styles.sectionCard} padding="xl">
          ...
          {categoryComparison.length > 0 ? (
            categoryComparison.map((cat: any, i: number) => {
              const maxCat = Math.max(cat.partner1Amount || 0, cat.partner2Amount || 0, 1);
              return (
                <View key={i} style={styles.compCategory}>
                  <Text style={[styles.compCatName, { color: colors.text.secondary }]}>
                    {cat.category || 'Category'}
                  </Text>
                  <View style={styles.compBarRow}>
                    <Text style={[styles.compBarLabel, { color: colors.text.tertiary }]}>
                      {p1.name?.slice(0, 3)}
                    </Text>
                    <View style={styles.compBarTrack}>
                      <View
                        style={[
                          styles.compBarFill,
                          {
                            width: `${((cat.partner1Amount || 0) / maxCat) * 100}%`,
                    ...
                    <View style={styles.compBarTrack}>
                      <View
                        style={[
                          styles.compBarFill,
                          {
                            width: `${((cat.partner2Amount || 0) / maxCat) * 100}%`,
              ...
              <View style={styles.barChartTrack}>
                <View
                  style={[
                    styles.barChartFill,
                    {
                      height: `${(incomeVsExpense / maxBar) * 100}%`,
                      backgroundColor: colors.status.success,
                    },
                  ]}
                />
              </View>
              <Text style={[styles.barChartValue, { color: colors.status.success }]}>
                {formatCurrency(incomeVsExpense)}
              </Text>
            </View>
            <View style={styles.barChartCol}>
              ...
              <View style={styles.barChartTrack}>
                <View
                  style={[
                    styles.barChartFill,
                    {
                      height: `${((d.monthlySpending || 0) / maxBar) * 100}%`,
                      backgroundColor: colors.status.error,
                    },
                  ]}
                />
              </View>
              <Text style={[styles.barChartValue, { color: colors.status.error }]}>
                {formatCurrency(d.monthlySpending || 0)}
              </Text>
            </View>
          </View>
        ...
        <Card variant="glass" style={styles.sectionCard} padding="xl">
          ...
          <View style={[styles.insightCard, { backgroundColor: colors.bg.glass }]}>
            ...
            <Text style={[styles.insightText, { color: colors.text.secondary }]}>
              {d.insightFairContribution ||
                'Based on your income ratio, fair contribution would be split proportionally.'}
            </Text>
          </View>
          <View style={[styles.insightCard, { backgroundColor: colors.bg.glass }]}>
            <Ionicons name="pricetag-outline" size={20} color={colors.status.warning} />
            <Text style={[styles.insightText, { color: colors.text.secondary }]}>
              {d.insightSubscriptions ||
                'You could save more by sharing subscriptions and memberships.'}
            </Text>
          </View>
        </Card>
        ...
        <Card variant="glass" style={styles.sectionCard} padding="xl">
          ...
          {sharedExpenses.length > 0 ? (
            sharedExpenses.slice(0, 5).map((exp: any, i: number) => (
              <TouchableOpacity
                key={exp.id || i}
                style={[styles.expenseRow, { borderBottomColor: colors.border.subtle }]}
              >
                ...
                <View style={styles.expenseInfo}>
                  <Text style={[styles.expenseName, { color: colors.text.primary }]}>
                ...
                <Text style={[styles.expenseAmount, { color: colors.status.error }]}>
        ...
        <View style={styles.actionsRow}>
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('CreateGroupExpense')}
          >
