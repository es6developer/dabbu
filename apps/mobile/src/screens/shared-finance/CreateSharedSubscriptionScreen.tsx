...
export function CreateSharedSubscriptionScreen() {
  ...
  async function loadMembers() {
    try {
      ...
      if (memberList.length > 0) setPaidBy(memberList[0].id);
    } catch (e) {
  ...
  function autoCalculateShares() {
    ...
    setMembers((prev) => prev.map((m) => (m.id === paidBy ? { ...m, share: '0', shareType: 'fixed' as const } : { ...m, share: equalShare, shareType: 'equal' as const })));
  }
  ...
  const categories = ['Entertainment', 'Productivity', 'Utilities', 'Cloud', 'Music', 'Video', 'Other'];
  ...
  return (
    <KeyboardAvoidingView style={[styles.container, { backgroundColor: colors.bg.primary }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <Card variant="glass" style={styles.sectionCard} padding="xl">
          ...
          <View style={styles.cycleRow}>
            {BILLING_CYCLES.map((cycle) => (
              <TouchableOpacity
                key={cycle}
                style={[styles.cycleBtn, { borderColor: colors.border.subtle }, billingCycle === cycle && { backgroundColor: colors.accent.primary + '20', borderColor: colors.accent.primary }]}
                onPress={() => setBillingCycle(cycle)}
              >
                <Text style={[styles.cycleBtnText, { color: colors.text.secondary }, billingCycle === cycle && { color: colors.accent.primary, fontWeight: '700' }]}>{cycle}</Text>
              </TouchableOpacity>
            ))}
          </View>
        ...
        <Card variant="glass" style={styles.sectionCard} padding="xl">
          ...
          <View style={styles.paidByRow}>
            {members.map((m) => (
              <TouchableOpacity
                key={m.id}
                style={[styles.paidByBtn, { borderColor: colors.border.subtle }, paidBy === m.id && { backgroundColor: colors.accent.primary + '20', borderColor: colors.accent.primary }]}
                onPress={() => setPaidBy(m.id)}
              >
                <Text style={[styles.paidByBtnText, { color: colors.text.secondary }, paidBy === m.id && { color: colors.accent.primary, fontWeight: '600' }]}>
                  {m.name || 'Member'}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <View style={styles.sharesHeader}>
            ...
            <TouchableOpacity onPress={autoCalculateShares}>
              <Text style={[styles.autoCalcText, { color: colors.accent.primary }]}>Auto Calculate</Text>
            </TouchableOpacity>
          </View>

          {members.map((m) => (
            <View key={m.id} style={styles.shareRow}>
              <Text style={[styles.shareName, { color: colors.text.secondary }]}>{m.name || 'Member'}</Text>
              <TextInput
                style={[styles.shareInput, { backgroundColor: colors.bg.tertiary, color: colors.text.primary, borderColor: colors.border.subtle }]}
                value={m.share}
        ...
        <Card variant="glass" style={styles.sectionCard} padding="xl">
          <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>Additional Settings</Text>
          ...
          <View style={styles.categoryRow}>
            {categories.map((cat) => (
              <TouchableOpacity
                key={cat}
                style={[styles.categoryChip, { borderColor: colors.border.subtle }, category === cat && { backgroundColor: colors.accent.primary + '20', borderColor: colors.accent.primary }]}
                onPress={() => setCategory(category === cat ? '' : cat)}
              >
                <Text style={[styles.categoryChipText, { color: colors.text.tertiary }, category === cat && { color: colors.accent.primary }]}>{cat}</Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          <View style={styles.reminderRow}>
            {REMINDER_OPTIONS.map((days) => (
              <TouchableOpacity
                key={days}
                style={[styles.reminderBtn, { borderColor: colors.border.subtle }, reminderDays === days && { backgroundColor: colors.accent.primary + '20', borderColor: colors.accent.primary }]}
                onPress={() => setReminderDays(days)}
              >
                <Text style={[styles.reminderBtnText, { color: colors.text.tertiary }, reminderDays === days && { color: colors.accent.primary }]}>{days}d</Text>
              </TouchableOpacity>
            ))}
          </View>
        </Card>
        ...
        <TouchableOpacity
          style={[styles.saveBtn, { backgroundColor: colors.accent.primary }, (!name.trim() || !amount.trim() || saving) && { opacity: 0.5 }]}
          onPress={handleSave}
          disabled={!name.trim() || !amount.trim() || saving}
        >
          {saving ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.saveBtnText}>Save Subscription</Text>}
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  cycleBtn: { flex: 1, paddingVertical: 10, borderRadius: 12, borderWidth: 1, alignItems: 'center' },
  ...
  sharesHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 },
  ...
  shareInput: { width: 80, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, fontSize: 14, borderWidth: 1, textAlign: 'center' },
  ...
  saveBtn: { marginHorizontal: 16, paddingVertical: 16, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginTop: 8 },
