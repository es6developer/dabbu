...
export function CoupleFinanceScreen() {
    ...
    async (refresh = false) => {
      ...
      try {
        ...
        const [dashboard, incomesData, expenseList] = await Promise.all([
          ...
          api.get<any>(`/shared-finance/groups/${groupId}/couple/incomes`).catch(() => ({ incomes: [], summary: { totalMonthly: 0 } })),
          api.get<any[]>(`/shared-finance/groups/${groupId}/expenses`).catch(() => []),
        ]);
        setData({
          ...(dashboard || {}),
          incomes: incomesData?.incomes || [],
          incomeSummary: incomesData?.summary || { totalMonthly: 0, partner1Total: 0, partner2Total: 0 },
  ...
  const allActivity = [
    ...
    ...incomes.map((i: any) => ({ ...i, _type: 'income' as const, description: i.source || 'Income', paidBy: i.createdBy })),
  ...
  if (!profile) {
    return (
      <ScrollView style={[s.screen, { backgroundColor: colors.bg.primary }]}
    ...
    <View style={[s.screen, { backgroundColor: colors.bg.primary }]}>
      ...
      >
        ...
        >
          <View style={s.headerRow}>
            ...
            <TouchableOpacity style={s.headerBtn} onPress={() => navigation.navigate('CoupleReports', { groupId })}>
        ...
        {activeTab === 'overview' ? (
          <View style={{ paddingHorizontal: 20, gap: 14, paddingTop: 8 }}>
            ...
            <View    style={s.heroFinanceCard}>
              ...
              <View style={[s.netRow, { backgroundColor: totalIncome >= totalExpenses ? 'rgba(52,199,89,0.15)' : 'rgba(255,77,79,0.15)' }]}>
                <Ionicons name={totalIncome >= totalExpenses ? 'trending-up' : 'trending-down'} size={14} color={totalIncome >= totalExpenses ? '#34C759' : '#FF4D4F'} />
                <Text style={[s.netText, { color: totalIncome >= totalExpenses ? '#34C759' : '#FF4D4F' }]}>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              ...
              <View style={[s.statCard, { backgroundColor: colors.bg.card, flex: 1 }]}>
                ...
                <Text style={[s.statValue, { color: colors.text.primary }]}>{fmt(savingsSaved)}</Text>
              ...
              <View style={[s.statCard, { backgroundColor: colors.bg.card, flex: 1 }]}>
                ...
                <Text style={[s.statValue, { color: colors.text.primary }]}>{totalExpenses > 0 ? expenses.length : 0}</Text>
            ...
            {totalIncome > 0 && totalExpenses > 0 && (
              <View style={[s.card, { backgroundColor: colors.bg.card }]}>
                <Text style={[s.cardTitle, { color: colors.text.primary }]}>Income vs Expenses</Text>
                <View style={{ marginTop: 10, gap: 10 }}>
                  <View>
                    ...
                    <View style={[s.vsBarOuter, { backgroundColor: colors.bg.tertiary }]}>
                      <View style={[s.vsBarFill, { width: `${Math.min((totalIncome / Math.max(totalIncome, totalExpenses)) * 100, 100)}%`, backgroundColor: '#34C759' }]} />
                    </View>
                  </View>
                  <View>
                    <View style={s.vsLabelRow}>
                    ...
                    <View style={[s.vsBarOuter, { backgroundColor: colors.bg.tertiary }]}>
                      <View style={[s.vsBarFill, { width: `${Math.min((totalExpenses / Math.max(totalIncome, totalExpenses)) * 100, 100)}%`, backgroundColor: '#FF4D4F' }]} />
                    </View>
                  </View>
                </View>
              </View>
            ...
            {monthlyOverview && (
              <View style={[s.card, { backgroundColor: colors.bg.card }]}>
                <View style={s.cardHeader}>
                  <Text style={[s.cardTitle, { color: colors.text.primary }]}>Monthly Comparison</Text>
                ...
                <View style={{ marginTop: 8, gap: 6 }}>
                  <View style={s.compRow}>
                    ...
                    <Text style={[s.compValue, { color: colors.text.primary }]}>{fmt(monthlyOverview.currentMonthTotal || 0)}</Text>
                  </View>
                  <View style={s.compRow}>
                    <Text style={[s.compLabel, { color: colors.text.tertiary }]}>Last Month</Text>
                    <Text style={[s.compValue, { color: colors.text.primary }]}>{fmt(monthlyOverview.lastMonthTotal || 0)}</Text>
                  </View>
                </View>
              </View>
            )}
            ...
            {insights.length > 0 && (
              <View style={{ gap: 8 }}>
                {insights.map((insight: string, i: number) => {
                  const isPositive = insight.includes('less') || insight.includes('Great') || insight.includes('on track') || insight.includes('decreased');
                  return (
                    <View key={i} style={[s.insightCard, { backgroundColor: isPositive ? '#34C75912' : '#F59E0B12' }]}>
                      <Ionicons name={isPositive ? 'bulb-outline' : 'trending-up-outline'} size={16} color={isPositive ? '#34C759' : '#F59E0B'} />
              ...
              <View style={[s.card, { backgroundColor: colors.bg.card }]}>
                ...
                {goals.slice(0, 3).map((goal: any, i: number) => (
                  <View key={goal.id || i} style={{ marginTop: 12 }}>
                    ...
                    <View style={[s.progressBar, { backgroundColor: colors.bg.tertiary, marginTop: 6 }]}>
                      <View style={[s.progressFill, { width: `${Math.min(goal.progress, 100)}%` }]} />
                    </View>
                  </View>
                ))}
              </View>
          ...
          <View style={{ paddingHorizontal: 20, gap: 10, paddingTop: 8 }}>
            {allActivity.length === 0 ? (
              <View style={s.emptyWrap}>
                ...
                <Text style={[s.emptyTitle, { color: colors.text.secondary }]}>No activity yet</Text>
                <Text style={[s.emptyDesc, { color: colors.text.tertiary }]}>Add an expense or income to get started</Text>
              </View>
            ) : (
              allActivity.slice(0, 30).map((item: any, i: number) => {
                ...
                const actorName = item.paidBy === partner1?.id ? partner1Name : item.paidBy === partner2?.id ? partner2Name : 'You';
                return (
                  <TouchableOpacity key={item.id || i} activeOpacity={0.7}
                  ...
                  >
                    ...
                    <Text style={[s.activityAmount, { color: isIncome ? '#34C759' : colors.text.primary }]}>
      ...
      >
        <TouchableOpacity
          activeOpacity={0.85}
          onPress={() => {
            Alert.alert('Add to Couple Finance', '', [
              { text: 'Cancel', style: 'cancel' },
              {
                text: 'Add Expense',
                onPress: () => navigation.navigate('SharedExpenseForm', { groupId, groupName: 'Couple' }),
              },
              {
                text: 'Add Income',
                onPress: () => navigation.navigate('CoupleIncome', { groupId }),
              },
            ]);
          }}
          style={s.addFabTouch}
...
const s = StyleSheet.create({
  ...
  headerBtn: { width: 34, height: 34, borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  ...
  tabRow: {
    flexDirection: 'row', marginHorizontal: 20, marginTop: 16, gap: 0,
  ...
  card: { borderRadius: 20, padding: 18, gap: 4, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 10, elevation: 2 },
  ...
  statCard: { borderRadius: 18, padding: 14, gap: 6, alignItems: 'center', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 6, elevation: 1 },
  ...
  insightCard: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 14, borderRadius: 14 },
