...
function transformExpense(raw: any): Expense {
  const paidByName = raw.paidBy?.user
    ? `${raw.paidBy.user.firstName || ''} ${raw.paidBy.user.lastName || ''}`.trim()
    : raw.paidBy?.name || 'Unknown';

  const shares: Share[] = (raw.splits || []).map((s: any) => {
    const memberName = s.member?.user
      ? `${s.member.user.firstName || ''} ${s.member.user.lastName || ''}`.trim()
      : s.member?.name || s.memberName || 'Unknown';
    return {
      memberId: s.memberId || s.member?.id || '',
      memberName,
      amount: Number(s.amount ?? 0),
      percentage: s.percentage ? Number(s.percentage) : undefined,
      settled: s.isSettled ?? s.settled ?? false,
    };
  });

  const comments: Comment[] = (raw.comments || []).map((c: any) => ({
    id: c.id,
    content: c.content,
  ...
  return {
    ...
    notes: raw.notes,
    paidBy: { id: raw.paidBy?.id || '', name: paidByName, avatar: raw.paidBy?.user?.avatarUrl },
    shares,
    comments,
...
export function GroupExpenseDetailScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      loadExpense();
    }, [accessToken, groupId, expenseId]),
  );

  async function loadExpense(isRefresh = false) {
    try {
      if (isRefresh) setRefreshing(true);
      else setLoading(true);
      const res = await api.get<any>(`/shared-finance/groups/${groupId}/expenses/${expenseId}`);
      setExpense(transformExpense(res.data || res));
    } catch (e: any) {
  ...
  async function handleAddComment() {
    ...
    try {
      ...
      const newComment: Comment = {
        ...
        user: {
          id: res.data?.user?.id || res.user?.id || '',
          name: res.data?.user
            ? `${res.data.user.firstName || ''} ${res.data.user.lastName || ''}`.trim() || 'You'
            : 'You',
          avatar: res.data?.user?.avatarUrl || res.user?.avatarUrl,
        },
      };
      setExpense((prev) => (prev ? { ...prev, comments: [...prev.comments, newComment] } : prev));
      setCommentText('');
    } catch (e: any) {
  ...
  if (!expense) {
    return (
      <View style={[styles.center, { backgroundColor: colors.bg.primary }]}>
        ...
        <Text style={[styles.errorText, { color: colors.status.error, marginTop: 12 }]}>
          Expense not found
        </Text>
      </View>
    );
  ...
  const settledShares = expense.shares.filter((s) => s.settled).length;
  const totalShares = expense.shares.length;
  const settledPercent = totalShares > 0 ? (settledShares / totalShares) * 100 : 0;
  const sharesSum = expense.shares.reduce((sum, s) => sum + Number(s.amount ?? 0), 0);
    ...
    >
      <View style={[styles.amountCard, { backgroundColor: colors.bg.tertiary }]}>
        <View style={[styles.glowRing, { backgroundColor: colors.accent.primary + '12' }]}>
          <View style={[styles.categoryIconWrap, { backgroundColor: colors.accent.primary + '25' }]}>
      ...
      <View style={styles.section}>
        ...
        {totalShares > 0 && (
          <View style={[styles.progressTrack, { backgroundColor: colors.bg.tertiary }]}>
            <View
              style={[styles.progressFill, { width: `${settledPercent}%`, backgroundColor: colors.status.success }]}
            />
          </View>
        )}
        {expense.shares.map((share, i) => {
          ...
          return (
            ...
            >
              <View style={[styles.shareColorDot, { backgroundColor: SPLIT_COLORS[i % SPLIT_COLORS.length] }]} />
              <View style={[styles.avatarSmall, { backgroundColor: SPLIT_COLORS[i % SPLIT_COLORS.length] + '25' }]}>
                <Text style={[styles.avatarTextSmall, { color: SPLIT_COLORS[i % SPLIT_COLORS.length] }]}>
        ...
        {totalShares === 0 && (
          <View style={styles.emptySection}>
            <Text style={[styles.emptyText, { color: colors.text.tertiary }]}>No split data</Text>
          </View>
        )}
      </View>
      ...
      {expense.notes ? (
        <View style={styles.section}>
          ...
          <View style={[styles.notesCard, { backgroundColor: colors.bg.card, borderColor: colors.border.subtle }]}>
            <Ionicons name="document-text-outline" size={16} color={colors.text.tertiary} />
            <Text style={[styles.notesText, { color: colors.text.secondary }]}>{expense.notes}</Text>
          </View>
        </View>
      ) : null}
      ...
      <View style={styles.section}>
        ...
        {expense.comments.length === 0 ? (
          <View style={styles.emptyComments}>
            <Ionicons name="chatbubble-ellipses-outline" size={28} color={colors.text.tertiary + '50'} />
...
const styles = StyleSheet.create({
  ...
  glowRing: { width: 72, height: 72, borderRadius: 24, alignItems: 'center', justifyContent: 'center', marginBottom: 16 },
  categoryIconWrap: { width: 56, height: 56, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
  ...
  splitBadge: { flexDirection: 'row', alignItems: 'center', paddingVertical: 5, paddingHorizontal: 12, borderRadius: 20, gap: 5 },
  splitBadgeText: { fontSize: 12, fontWeight: '600', textTransform: 'capitalize' },
  dateBadge: { flexDirection: 'row', alignItems: 'center', paddingVertical: 5, paddingHorizontal: 12, borderRadius: 20, gap: 5 },
  section: { paddingHorizontal: 16, marginBottom: 24 },
  sectionTitle: { fontSize: 12, textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 12, fontWeight: '600' },
  ...
  paidByCard: { flexDirection: 'row', alignItems: 'center', padding: 16, borderRadius: 18, gap: 14 },
  avatar: { width: 48, height: 48, borderRadius: 24, alignItems: 'center', justifyContent: 'center' },
  avatarText: { fontSize: 20, fontWeight: '700' },
  paidByInfo: { flex: 1 },
  paidByName: { fontSize: 16, fontWeight: '600' },
  paidByRole: { fontSize: 12, marginTop: 2 },
  paidMarker: { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  ...
  shareRow: { flexDirection: 'row', alignItems: 'center', padding: 14, borderRadius: 16, marginBottom: 8, borderWidth: 1, gap: 10 },
  shareInfo: { flex: 1 },
  ...
  notesCard: { flexDirection: 'row', padding: 14, borderRadius: 14, borderWidth: 1, gap: 10, alignItems: 'flex-start' },
  ...
  commentInputRow: { flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: 16, paddingVertical: 12, gap: 10, borderTopWidth: 1 },
  commentInput: { flex: 1, paddingVertical: 10, paddingHorizontal: 16, borderRadius: 14, fontSize: 14, borderWidth: 1, maxHeight: 80 },
