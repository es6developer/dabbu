...
import { View, Text, FlatList, StyleSheet, TouchableOpacity, ActivityIndicator, RefreshControl, Switch, Modal, TextInput } from 'react-native';
...
export function ContributionRulesScreen() {
  ...
  function openCreateModal() {
    ...
    members.forEach((m) => { initValues[m.id] = ''; });
  ...
  function getPreviewValues(): { memberName: string; value: string }[] {
    ...
    return members.map((m) => ({
      memberName: m.name,
      value: form.memberValues[m.id] ? `${form.memberValues[m.id]}${form.type === 'fixed' ? '' : '%'}` : '-',
    }));
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg.primary }]}>
      <FlatList
        ...
        ListHeaderComponent={
          <LinearGradient colors={['#1a1a2e', colors.bg.primary]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={styles.header}>
            <Text style={[styles.headerTitle, { color: colors.text.primary }]}>Contribution Rules</Text>
            <Text style={[styles.headerSub, { color: colors.text.tertiary }]}>{rules.length} rule{rules.length !== 1 ? 's' : ''} configured</Text>
          </LinearGradient>
        }
        renderItem={({ item }) => {
            ...
            <Card variant="glass" style={styles.ruleCard} padding="lg">
              <View style={styles.ruleHeader}>
                <View style={styles.ruleInfo}>
                  <Text style={[styles.ruleName, { color: colors.text.primary }]}>{item.name || 'Rule'}</Text>
                  <View style={[styles.ruleBadge, { backgroundColor: badge.color + '20' }]}>
                    <Text style={[styles.ruleBadgeText, { color: badge.color }]}>{badge.label}</Text>
                  </View>
                </View>
                <Switch
                  value={item.active}
              ...
              {Array.isArray(item.breakdown) && item.breakdown.map((b: any, i: number) => (
                <View key={i} style={[styles.breakdownRow, { borderBottomColor: colors.border.subtle }]}>
                  <Text style={[styles.breakdownName, { color: colors.text.secondary }]}>{b.name || 'Member'}</Text>
              ...
              <TouchableOpacity
                style={[styles.applyBtn, { borderTopColor: colors.border.subtle, backgroundColor: colors.accent.primary + '10' }]}
                onPress={() => handleApplyRule(item.id)}
              >
                <Ionicons name="flash-outline" size={16} color={colors.accent.primary} />
                <Text style={[styles.applyBtnText, { color: colors.accent.primary }]}>Apply Rule</Text>
              </TouchableOpacity>
            </Card>
          );
        }}
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptyDesc, { color: colors.text.tertiary }]}>Create contribution rules to split expenses fairly</Text>
          </View>
        }
        ListFooterComponent={
          <TouchableOpacity style={[styles.createBtn, { borderColor: colors.border.default }]} onPress={openCreateModal}>
            <Ionicons name="add-circle-outline" size={20} color={colors.accent.primary} />
            <Text style={[styles.createBtnText, { color: colors.accent.primary }]}>Create Rule</Text>
          </TouchableOpacity>
        }
      />
              ...
              <View style={styles.modalBody}>
                ...
                <View style={styles.typeRow}>
                  {ruleTypes.map((rt) => (
                    <TouchableOpacity
                      key={rt.value}
                      style={[styles.typeBtn, { borderColor: colors.border.subtle }, form.type === rt.value && { backgroundColor: colors.accent.primary + '20', borderColor: colors.accent.primary }]}
                      onPress={() => setForm((prev) => ({ ...prev, type: rt.value }))}
                    >
                      <Ionicons name={rt.icon} size={18} color={form.type === rt.value ? colors.accent.primary : colors.text.tertiary} />
                      <Text style={[styles.typeBtnText, { color: colors.text.tertiary }, form.type === rt.value && { color: colors.accent.primary }]}>{rt.label}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                ...
                {members.map((m) => (
                  <View key={m.id} style={styles.memberValueRow}>
                    <Text style={[styles.memberValueName, { color: colors.text.secondary }]}>{m.name}</Text>
                    <TextInput
                      style={[styles.memberValueInput, { backgroundColor: colors.bg.tertiary, color: colors.text.primary, borderColor: colors.border.subtle }]}
                ...
                <View style={[styles.previewBox, { backgroundColor: colors.bg.glass, borderColor: colors.border.subtle }]}>
                  <Text style={[styles.previewTitle, { color: colors.text.primary }]}>Preview</Text>
                  {getPreviewValues().map((pv, i) => (
                    <View key={i} style={[styles.previewRow, { borderBottomColor: colors.border.subtle }]}>
                      <Text style={[styles.previewName, { color: colors.text.secondary }]}>{pv.memberName}</Text>
                      <Text style={[styles.previewValue, { color: colors.accent.primary }]}>{pv.value}</Text>
                    </View>
                  ))}
                </View>

                <TouchableOpacity
                  style={[styles.saveBtn, { backgroundColor: colors.accent.primary }, (!form.name.trim() || saving) && { opacity: 0.5 }]}
                  onPress={handleSave}
                  disabled={!form.name.trim() || saving}
                >
                  {saving ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.saveBtnText}>Save Rule</Text>}
                </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  applyBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 12, marginTop: 12, borderRadius: 12 },
  ...
  createBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginHorizontal: 16, marginTop: 8, paddingVertical: 16, borderRadius: 16, borderWidth: 1 },
  createBtnText: { fontSize: 15, fontWeight: '600' },
  modalContainer: { flex: 1 },
  modalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 16, paddingTop: 60 },
  modalTitle: { fontSize: 17, fontWeight: '600' },
  modalBody: { padding: 20 },
  label: { fontSize: 12, fontWeight: '600', marginBottom: 8, marginTop: 16 },
  input: { borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, fontSize: 15, borderWidth: 1, marginBottom: 4 },
  typeRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  typeBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, borderWidth: 1 },
  ...
  memberValueInput: { width: 100, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 8, fontSize: 14, borderWidth: 1, textAlign: 'center' },
  previewBox: { borderRadius: 14, borderWidth: 1, padding: 16, marginTop: 16 },
  previewTitle: { fontSize: 14, fontWeight: '600', marginBottom: 10 },
  previewRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: StyleSheet.hairlineWidth },
  previewName: { fontSize: 13, fontWeight: '500' },
  previewValue: { fontSize: 13, fontWeight: '700' },
  saveBtn: { paddingVertical: 16, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginTop: 20 },
