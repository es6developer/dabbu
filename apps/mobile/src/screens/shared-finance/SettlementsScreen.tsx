...
export function SettlementsScreen() {
  ...
  async function handleMarkPaid(settlementId: string) {
    ...
    try {
      ...
      await api.patch(`/shared-finance/groups/${groupId}/settlements/${settlementId}`, {
        status: 'completed',
      });
      setSettlements((prev) =>
        prev.map((s) =>
      ...
      <View style={[styles.summaryCard, { backgroundColor: colors.bg.tertiary }]}>
        <View style={styles.summaryRow}>
          ...
          <View style={styles.summaryItem}>
            ...
            <Text style={[styles.summaryAmount, { color: colors.status.success }]}>
      ...
      <View style={[styles.segmentedControl, { backgroundColor: colors.bg.card }]}>
        {(['pending', 'completed'] as SettlementTab[]).map((t) => (
          <TouchableOpacity
            ...
            style={[
              styles.segment,
              { backgroundColor: tab === t ? colors.accent.primary : 'transparent' },
            ]}
            onPress={() => setTab(t)}
          >
            <Text
              style={[styles.segmentText, { color: tab === t ? '#fff' : colors.text.secondary }]}
            >
        ...
        {filtered.length === 0 ? (
          <View style={styles.emptyState}>
            <View style={[styles.emptyIconWrap, { backgroundColor: colors.bg.card }]}>
              <Ionicons
                name={tab === 'pending' ? 'checkmark-done-outline' : 'cash-outline'}
                size={40}
            ...
            <Text style={[styles.emptySub, { color: colors.text.tertiary }]}>
              {tab === 'pending'
                ? 'All caught up! Everyone is settled up.'
              ...
              <View style={styles.settlementTop}>
                <View style={styles.settlementUsers}>
                  <View style={styles.userPair}>
                    <View
                      style={[styles.userAvatar, { backgroundColor: colors.status.errorLight }]}
                    >
                      <Text style={[styles.userAvatarText, { color: colors.status.error }]}>
                        {(settlement.fromUser.name || '?')[0].toUpperCase()}
                      </Text>
                    </View>
                    <Text
                  ...
                  <View style={styles.userPair}>
                    <View
                      style={[styles.userAvatar, { backgroundColor: colors.status.successLight }]}
                    >
                      <Text style={[styles.userAvatarText, { color: colors.status.success }]}>
                        {(settlement.toUser.name || '?')[0].toUpperCase()}
                      </Text>
                    </View>
                    <Text
                      style={[styles.userName, { color: colors.text.secondary }]}
                ...
                >
                  {marking === settlement.id ? (
                    ...
                    <>
                      ...
                      <Text style={[styles.markPaidText, { color: colors.status.success }]}>
                        Mark as Paid
                      </Text>
                    </>
                  )}
