...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, ActivityIndicator, Alert, Share, Platform } from 'react-native';
...
export function InviteMembersScreen() {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.bg.primary }]}>
      ...
      <Text style={[styles.groupName, { color: colors.text.secondary }]}>{groupName || 'Group'}</Text>

      <View style={[styles.codeCard, { backgroundColor: colors.bg.tertiary }]}>
        <View style={[styles.qrPlaceholder, { backgroundColor: colors.bg.card, borderColor: colors.border.subtle }]}>
        ...
        <View style={[styles.codeRow, { backgroundColor: colors.bg.card, borderColor: colors.border.subtle }]}>
          <Text style={[styles.codeText, { color: colors.accent.primary }]}>{inviteCode || '------'}</Text>
        </View>

        <View style={styles.codeActions}>
          <TouchableOpacity
            style={[styles.codeActionBtn, { backgroundColor: copied ? colors.status.successLight : colors.bg.card, borderColor: colors.border.subtle }]}
            onPress={handleCopy}
          >
            <Ionicons
            ...
            />
            <Text style={[styles.codeActionText, { color: copied ? colors.status.success : colors.text.primary }]}>
              {copied ? 'Copied' : 'Copy Code'}
            </Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.codeActionBtn, { backgroundColor: colors.status.infoLight, borderColor: colors.border.subtle }]}
            onPress={() => {
        ...
        {generating ? (
          ...
          <TouchableOpacity style={styles.generateRow} onPress={handleGenerateCode}>
            ...
            <Text style={[styles.generateText, { color: colors.text.tertiary }]}>Generate new code</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={styles.membersSection}>
        <View style={styles.membersHeader}>
          <Text style={[styles.membersTitle, { color: colors.text.secondary }]}>Current Members ({members.length})</Text>
        </View>
        {members.map((m, i) => (
          <View key={m.id || i} style={[styles.memberRow, { borderBottomColor: colors.border.subtle }]}>
            <View style={[styles.memberAvatar, { backgroundColor: colors.accent.primary + '25' }]}>
              <Text style={[styles.memberAvatarText, { color: colors.accent.primary }]}>{(m.name || '?')[0].toUpperCase()}</Text>
            </View>
            <View style={styles.memberInfo}>
              <Text style={[styles.memberName, { color: colors.text.primary }]}>{m.name}</Text>
              {m.email && <Text style={[styles.memberEmail, { color: colors.text.tertiary }]}>{m.email}</Text>}
            </View>
            {m.role && (
...
const styles = StyleSheet.create({
  ...
  codeCard: { marginHorizontal: 20, padding: 24, borderRadius: 24, alignItems: 'center', marginBottom: 20 },
  qrPlaceholder: { width: 180, height: 180, borderRadius: 16, borderWidth: 1, alignItems: 'center', justifyContent: 'center', marginBottom: 20, position: 'relative', overflow: 'hidden' },
  qrGrid: { flexDirection: 'row', flexWrap: 'wrap', width: 140, height: 140, alignContent: 'center', justifyContent: 'center', gap: 2 },
  qrDot: { width: 16, height: 16, borderRadius: 3 },
  qrOverlay: { position: 'absolute', width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
  inviteLabel: { fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 },
  codeRow: { paddingVertical: 12, paddingHorizontal: 24, borderRadius: 12, borderWidth: 1, marginBottom: 16 },
  codeText: { fontSize: 22, fontWeight: '700', letterSpacing: 2, fontFamily: Platform.select({ ios: 'Menlo', android: 'monospace', default: 'monospace' }) },
  codeActions: { flexDirection: 'row', gap: 10, width: '100%' },
  codeActionBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 12, borderRadius: 12, borderWidth: 1, gap: 6 },
  ...
  shareBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginHorizontal: 20, paddingVertical: 16, borderRadius: 16, gap: 8, marginBottom: 20 },
