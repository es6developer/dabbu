  ...
  const loadData = useCallback(
    async (silent = false, refresh = false) => {
      ...
      try {
        ...
        const tripP = api
          .get<any>(`/shared-finance/groups/${groupId}/trip/dashboard`)
          .finally(tick);
        ...
        if (tripRes.status === 'fulfilled') {
          setTrip(tripRes.value?.data || tripRes.value);
        }
        ...
        <View style={s.content}>
            ...
            <>
              ...
              <View style={[s.budgetCard, { backgroundColor: colors.bg.secondary }]}>
                <View style={s.budgetRow}>
                  ...
                  <Text style={[s.budgetValue, { color: colors.text.primary }]}>
                    {fmt(totalSpent)}
                  </Text>
                </View>
                <View style={s.budgetRow}>
                        ...
                        <View style={s.barTrack}>
                          <View
                            style={[
                              ...
                              {
                                width: `${pct}%`,
                                backgroundColor: CATEGORY_COLORS[cat] || colors.text.tertiary,
                              },
                            ]}
                          />
                        </View>
          ...
          {recentExpenses.length > 0 && (
            <>
              ...
              >
                <Text style={[s.sectionTitle, { color: colors.text.primary, marginBottom: 0 }]}>
                  Recent Expenses
                </Text>
                {groupId && (
                  <TouchableOpacity
              ...
              {recentExpenses.map((exp: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      <Text
                        style={[s.expenseDesc, { color: colors.text.primary }]}
                        numberOfLines={1}
                      >
                ...
                return (
                  ...
                  >
                    <View
                      style={[
                        s.balanceAvatar,
                        { backgroundColor: owes ? colors.status.error : colors.status.success },
                      ]}
                    >
                    ...
                    <Text
                      style={[
                        s.balanceAmt,
                        { color: owes ? colors.status.error : colors.status.success },
                      ]}
                    >
                      {owes ? 'Owes ' : 'Gets '}
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => {
                          ...
                          Linking.openURL(upiLink).catch(() =>
                            alertService.alert(
                              'Settle Up',
                              `Pay ${fmt(Math.abs(b.balance))} to ${b.name}`,
                            ),
                          );
                        }}
                      >
