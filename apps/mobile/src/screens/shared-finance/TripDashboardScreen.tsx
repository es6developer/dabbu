...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, ActivityIndicator, RefreshControl, Dimensions, Animated } from 'react-native';
...
export function TripDashboardScreen() {
  ...
  if (!dashboard) {
    return (
      <View style={[styles.loading, { backgroundColor: colors.bg.primary }]}>
        <Text style={[styles.errorText, { color: colors.text.tertiary }]}>Dashboard not available</Text>
      </View>
    );
  }
  ...
  const categoryBreakdown = Array.isArray(dashboard.categoryBreakdown) ? dashboard.categoryBreakdown : [];
      ...
      >
        <Card variant="glass" style={styles.sectionCard} padding="xl">
          <View style={styles.budgetOverview}>
            <Svg width={120} height={120} viewBox="0 0 120 120">
              <Circle cx="60" cy="60" r={ringRadius} stroke={colors.border.subtle} strokeWidth="10" fill="none" />
              <Circle
            ...
            <View style={styles.ringCenter}>
              <Text style={[styles.ringPct, { color: overspent ? colors.status.error : colors.text.primary }]}>{spentPct.toFixed(0)}%</Text>
              <Text style={[styles.ringLabel, { color: colors.text.tertiary }]}>spent</Text>
            </View>
            <View style={styles.budgetInfo}>
              <View style={styles.budgetRow}>
                <Text style={[styles.budgetLabel, { color: colors.text.tertiary }]}>Total Budget</Text>
                <Text style={[styles.budgetValue, { color: colors.text.primary }]}>{formatCurrency(totalBudget)}</Text>
              </View>
              <View style={styles.budgetRow}>
                <Text style={[styles.budgetLabel, { color: colors.text.tertiary }]}>Spent</Text>
                <Text style={[styles.budgetValue, { color: colors.status.error }]}>{formatCurrency(totalSpent)}</Text>
              </View>
              <View style={styles.budgetRow}>
                <Text style={[styles.budgetLabel, { color: colors.text.tertiary }]}>Remaining</Text>
                <Text style={[styles.budgetValue, { color: colors.status.success }]}>{formatCurrency(remaining)}</Text>
              </View>
            </View>
          </View>
        </Card>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={[styles.statCard, { backgroundColor: colors.bg.card, borderColor: colors.border.subtle }]}>
            ...
            <Text style={[styles.statValue, { color: colors.text.primary }]}>{formatCurrency(dailyAvg)}</Text>
        ...
        {categoryBreakdown.length > 0 && (
          <Card variant="glass" style={styles.sectionCard} padding="xl">
            <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>Category Breakdown</Text>
              ...
              return (
                <View key={cat.category || i} style={styles.catRow}>
                  <View style={styles.catHeader}>
                    <Text style={[styles.catName, { color: colors.text.secondary }]}>{cat.category || 'Other'}</Text>
                    <Text style={[styles.catAmount, { color: colors.text.primary }]}>{formatCurrency(cat.amount || 0)}</Text>
                  </View>
                  <View style={styles.catTrack}>
                    <View style={[styles.catFill, { width: `${Math.min(pct, 100)}%`, backgroundColor: cat.color || colors.accent.primary }]} />
                  </View>
                  <Text style={[styles.catPct, { color: colors.text.tertiary }]}>{pct.toFixed(1)}%</Text>
                </View>
              );
            })}
          </Card>
        ...
        {spendingTrend.length > 0 && (
          <Card variant="glass" style={styles.sectionCard} padding="xl">
            <Text style={[styles.sectionTitle, { color: colors.text.primary }]}>Spending Trend</Text>
            <View style={styles.trendChart}>
              {spendingTrend.map((point: any, i: number) => {
                const height = Math.max((Number(point.amount) / maxTrend) * 100, 4);
                return (
                  <View key={i} style={styles.trendCol}>
                    <View style={[styles.trendBar, { height, backgroundColor: colors.accent.primary }]} />
            ...
            {perPerson.map((person: any, i: number) => {
              ...
              return (
                <View key={person.id || i} style={styles.personRow}>
                  <Text style={[styles.personName, { color: colors.text.secondary }]}>{person.name || 'Member'}</Text>
                  <View style={styles.personBarTrack}>
                    <View style={[styles.personBarFill, { width: `${pct}%` }]} />
                  </View>
                  <Text style={[styles.personAmount, { color: colors.text.primary }]}>{formatCurrency(person.spent || 0)}</Text>
                </View>
              );
            })}
          </Card>
          ...
          <Card variant="glass" style={styles.sectionCard} padding="xl">
            ...
            {insights.map((insight: any, i: number) => (
              <View key={i} style={[styles.insightRow, { backgroundColor: colors.bg.glass }]}>
                ...
                <Text style={[styles.insightText, { color: colors.text.secondary }]}>{insight.message || insight}</Text>
              </View>
            ))}
          </Card>
        )}
        ...
        {overspent && (
          <View style={[styles.alertBanner, { backgroundColor: colors.status.errorLight, borderColor: colors.status.error }]}>
            <Ionicons name="warning-outline" size={20} color={colors.status.error} />
            <Text style={[styles.alertText, { color: colors.status.error }]}>Overspent by {formatCurrency(totalSpent - totalBudget)}</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  ringCenter: { position: 'absolute', left: 20, top: 20, width: 80, height: 80, justifyContent: 'center', alignItems: 'center' },
  ...
  statCard: { flex: 1, padding: 18, borderRadius: 16, borderWidth: 1, alignItems: 'center', gap: 6 },
  ...
  catTrack: { height: 8, borderRadius: 4, backgroundColor: 'rgba(255,255,255,0.06)', overflow: 'hidden', marginBottom: 4 },
  ...
  personBarTrack: { flex: 1, height: 8, borderRadius: 4, backgroundColor: 'rgba(255,255,255,0.06)', overflow: 'hidden' },
  ...
  insightRow: { flexDirection: 'row', alignItems: 'flex-start', padding: 12, borderRadius: 12, gap: 10, marginBottom: 8 },
  insightText: { fontSize: 13, lineHeight: 18, flex: 1 },
  alertBanner: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 16, marginBottom: 16, padding: 14, borderRadius: 14, borderWidth: 1, gap: 10 },
