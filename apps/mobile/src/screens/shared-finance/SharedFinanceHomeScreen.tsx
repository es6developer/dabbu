...
import {
  View, Text, FlatList, StyleSheet, TouchableOpacity, Alert, Animated, RefreshControl,
} from 'react-native';
...
const SPACE_TYPE_CONFIG: Record<string, { label: string; icon: string; gradient: [string, string] }> = {
...
const GROUP_TYPES = [
  { key: 'all', label: 'All' },
  ...Object.entries(SPACE_TYPE_CONFIG).filter(([k]) => k !== 'default').map(([k, v]) => ({ key: k, label: v.label })),
];
...
function computeFinancialSummary(groups: any[], currentUserId?: string) {
  let totalOwedToMe = 0, totalIOwe = 0, pendingSettlements = 0;
  groups.forEach((g) => {
  ...
  return (
    <View style={[sCard.netBadge, { backgroundColor: bg }]}>
      ...
      <Text style={[typography.caption, { color: fg, fontWeight: '700' }]}>
        {isPositive ? '+' : ''}{fmtCompact(Math.abs(net))}
      </Text>
    </View>
  );
}
      ...
      >
        <View style={sCard.topRow}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Text style={[typography.calloutBold, { color: colors.text.primary }]}>Financial Summary</Text>
          </View>
        ...
        <View style={sCard.metricsRow}>
          ...
          <View style={sCard.metric}>
            ...
            <Text style={[typography.expenseAmount, { color: net >= 0 ? colors.status.success : colors.status.error }]}>
        ...
        <View style={[sCard.netRow, { borderTopColor: colors.border.subtle }]}>
          ...
          {summary.pendingSettlements > 0 && (
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              ...
              <Text style={[typography.caption, { color: colors.status.warning, fontWeight: '500' }]}>
...
function GroupCard({ group, currentUserId, colors, typography, anim, onPress, onLongPress }: any) {
  ...
  const barColor = unsettledRatio < 0.3 ? colors.status.success
    : unsettledRatio < 0.7 ? colors.status.warning : colors.status.error;

  return (
    <Animated.View style={{ opacity: anim, transform: [{ translateY: anim.interpolate({ inputRange: [0, 1], outputRange: [32, 0] }) }] }}>
      <TouchableOpacity
            ...
            <View style={gCard.body}>
              {totalSpent === 0 ? (
                <View style={gCard.balanceRow}>
                  <View style={[gCard.balanceChip, { backgroundColor: colors.text.tertiary + '12' }]}>
                    ...
                    <Text style={[typography.caption, { color: colors.text.tertiary, fontWeight: '500' }]}>
                      No activity yet
                    </Text>
                  </View>
                </View>
                ...
                <View style={gCard.balanceRow}>
                  {owedToMe > 0 && (
                    <View style={[gCard.balanceChip, { backgroundColor: colors.status.success + '12' }]}>
                      <Ionicons name="arrow-down" size={12} color={colors.status.success} />
                      <Text style={[typography.caption, { color: colors.status.success, fontWeight: '600' }]}>
                        Owed {fmtCompact(owedToMe)}
                      </Text>
                    </View>
                  )}
                  {iOwe > 0 && (
                    <View style={[gCard.balanceChip, { backgroundColor: colors.status.error + '12' }]}>
                      <Ionicons name="arrow-up" size={12} color={colors.status.error} />
                      <Text style={[typography.caption, { color: colors.status.error, fontWeight: '600' }]}>
                        Owe {fmtCompact(iOwe)}
                      </Text>
                    </View>
                  )}
              ...
              <View style={[gCard.statsRow, { borderTopColor: colors.border.subtle }]}>
                <View style={gCard.stat}>
                  ...
                  <Text style={[typography.subheadBold, { color: colors.text.primary }]}>{fmtCompact(totalSpent)}</Text>
                </View>
                <View style={gCard.stat}>
                  <Text style={[typography.caption2, { color: colors.text.tertiary }]}>Members</Text>
                  <Text style={[typography.subheadBold, { color: colors.text.primary }]}>{memberCount}</Text>
                </View>
                <View style={gCard.stat}>
                  <Text style={[typography.caption2, { color: colors.text.tertiary }]}>Txns</Text>
                  <Text style={[typography.subheadBold, { color: colors.text.primary }]}>{txnCount}</Text>
                </View>
              </View>
                ...
                <View style={gCard.barOuter}>
                  <View style={[{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }]}>
                    <Text style={[typography.caption2, { color: colors.text.tertiary }]}>
                      {isSettled ? 'Fully settled' : unsettledRatio < 0.5 ? 'Nearly settled' : 'Needs attention'}
                    </Text>
                  ...
                  <View style={[gCard.barTrack, { backgroundColor: colors.bg.tertiary }]}>
                    <View style={[gCard.barFill, { width: `${Math.max(unsettledRatio * 100, 2)}%`, backgroundColor: barColor }]} />
                  </View>
                </View>
              )}
            </View>
...
export function SharedFinanceHomeScreen() {
  ...
  const loadData = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      const groupsData = groupsRes.status === 'fulfilled'
        ...
        : [];
      const goalsData = goalsRes.status === 'fulfilled'
        ? Array.isArray(goalsRes.value?.data) ? goalsRes.value.data : Array.isArray(goalsRes.value) ? goalsRes.value : []
        : [];
      ...
      if (!hasAnimated.current) {
        ...
        Animated.spring(headerAnim, { toValue: 1, tension: 70, friction: 12, useNativeDriver: true }).start();
      } else {
        headerAnim.setValue(1);
      }
    } catch (e: any) {
      if (!ctrl.signal.aborted && e.message !== 'Session expired. Please login again.') { setGroups([]); setGoals([]); }
    } finally {
  ...
  const financialSummary = useMemo(() => computeFinancialSummary(groups, user?.id), [groups, user?.id]);
  ...
  const goalTotal = useMemo(() => {
    ...
    goals.forEach((g) => { saved += Number(g.saved || g.currentAmount || 0); target += Number(g.target || g.targetAmount || 0); });
    ...
    return (
      <BaseScreen noPadding>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginHorizontal: H_PADDING, marginTop: 8 }}>
          {[0, 1, 2, 3, 4].map((i) => <Skeleton key={i} width={56 + i * 10} height={34} borderRadius={17} />)}
        </View>
  ...
  return (
    <BaseScreen noPadding>
      <FlatList
        ...
        contentContainerStyle={filtered.length === 0 ? { flexGrow: 1 } : { paddingBottom: insets.bottom + 100 }}
        ListHeaderComponent={
          <Animated.View style={{ opacity: Animated.multiply(headerAnim, headerOpacity), transform: [{ translateY: headerAnim.interpolate({ inputRange: [0, 1], outputRange: [12, 0] }) }], paddingHorizontal: H_PADDING }}>
            <View style={{ paddingTop: 4 }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                <Text style={[typography.sectionHeader, { color: colors.text.primary, flex: 1 }]}>Your Spaces</Text>
                <TouchableOpacity
                  style={{ width: 40, height: 40, borderRadius: 12, backgroundColor: colors.accent.primary, alignItems: 'center', justifyContent: 'center' }}
            ...
            <SummaryCard summary={financialSummary} anim={headerAnim} colors={colors} typography={typography} />
            ...
            {goals.length > 0 && (
              ...
              >
                ...
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                      <View style={{ width: 32, height: 32, borderRadius: 10, backgroundColor: colors.accent.primary + '22', alignItems: 'center', justifyContent: 'center' }}>
                        <Ionicons name="trophy-outline" size={16} color={colors.accent.primary} />
                      </View>
                      <Text style={[typography.cardTitle, { color: colors.text.primary }]}>Goal Progress</Text>
                    </View>
                    <Ionicons name="chevron-forward" size={16} color={colors.text.tertiary} />
                  </View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 10 }}>
                    <View style={{ flex: 1, height: 8, borderRadius: 999, backgroundColor: colors.bg.tertiary, overflow: 'hidden' }}>
                      <View style={{ width: `${goalTotal.pct}%`, height: '100%', borderRadius: 999, backgroundColor: colors.accent.primary }} />
                    </View>
                    <Text style={[typography.calloutBold, { color: colors.accent.primary }]}>{Math.round(goalTotal.pct)}%</Text>
                  </View>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 6 }}>
                    <Text style={[typography.caption, { color: colors.text.tertiary }]}>
                      <Text style={{ color: colors.status.success }}>{fmt(goalTotal.saved)}</Text> saved of {fmt(goalTotal.target)}
                    </Text>
                    <Text style={[typography.caption, { color: colors.text.tertiary, fontWeight: '600' }]}>
            ...
            <SearchSection value={search} onChangeText={setSearch} placeholder="Search spaces..." onClear={() => setSearch('')} />
            <FilterSection options={typeFilterOptions} selected={typeFilter} onSelect={setTypeFilter} />
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              <Text style={[typography.calloutBold, { color: colors.text.primary }]}>
                {filtered.length > 0 ? `${filtered.length} group${filtered.length > 1 ? 's' : ''}` : 'Groups'}
              </Text>
              <Text style={[typography.caption, { color: colors.text.tertiary }]}>
        ...
        renderItem={({ item }) => (
          <GroupCard
            group={item} currentUserId={user?.id} colors={colors} typography={typography}
        ...
        ListEmptyComponent={
          <View style={{ paddingHorizontal: H_PADDING, marginTop: 32 }}>
            <EmptyState
              ...
              message={search || typeFilter !== 'all' ? 'Try a different search or filter' : 'Create a space to split expenses with your people'}
...
const sCard = StyleSheet.create({
  ...
  netRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingTop: 12, marginTop: 12, borderTopWidth: 1 },
  netBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8 },
});

const gCard = StyleSheet.create({
  outer: { marginHorizontal: H_PADDING, marginBottom: 16, borderRadius: 16, borderWidth: 1, overflow: 'hidden' },
  card: { borderRadius: 16, overflow: 'hidden' },
  cover: { height: 100 },
  coverOverlay: { flex: 1, padding: 14, justifyContent: 'flex-end', backgroundColor: 'rgba(0,0,0,0.25)' },
  coverMeta: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', position: 'absolute', top: 10, left: 14, right: 14 },
  coverIcon: { width: 32, height: 32, borderRadius: 10, backgroundColor: 'rgba(255,255,255,0.2)', alignItems: 'center', justifyContent: 'center' },
  typeBadge: { paddingHorizontal: 10, paddingVertical: 3, borderRadius: 8, backgroundColor: 'rgba(255,255,255,0.2)' },
  ...
  balanceChip: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8 },
