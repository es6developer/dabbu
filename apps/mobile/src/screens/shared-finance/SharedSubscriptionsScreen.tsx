...
import { View, Text, FlatList, StyleSheet, TouchableOpacity, ActivityIndicator, RefreshControl, Switch } from 'react-native';
...
export function SharedSubscriptionsScreen() {
  ...
  const totalMonthly = subscriptions.reduce((s, sub) => {
    ...
    if (sub.billingCycle === 'yearly') return s + amount / 12;
    if (sub.billingCycle === 'quarterly') return s + amount / 3;
    return s + amount;
  }, 0);
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <>
            <LinearGradient colors={['#1a1a2e', colors.bg.primary]} start={{ x: 0, y: 0 }} end={{ x: 0, y: 1 }} style={styles.header}>
              <Text style={[styles.headerTitle, { color: colors.text.primary }]}>Shared Subscriptions</Text>
              <Text style={[styles.headerSub, { color: colors.text.tertiary }]}>
                {subscriptions.filter((s) => s.active).length} active · {formatCurrency(Math.round(totalMonthly))}/mo
              </Text>
            </LinearGradient>

            {upcoming.length > 0 && (
              <Card variant="glass" style={styles.upcomingCard} padding="md">
                <Text style={[styles.upcomingTitle, { color: colors.text.primary }]}>Upcoming Renewals</Text>
                {upcoming.map((item: any, i: number) => (
                  <View key={item.id || i} style={[styles.upcomingRow, { borderBottomColor: colors.border.subtle }]}>
                    <Ionicons name="alarm-outline" size={16} color={colors.status.warning} />
                    <Text style={[styles.upcomingName, { color: colors.text.secondary }]}>{item.name}</Text>
          ...
          return (
            <Card variant="glass" style={styles.subCard} padding="lg">
              <View style={styles.subHeader}>
                <View style={[styles.subIconWrap, { backgroundColor: colors.accent.primary + '15' }]}>
                  <Ionicons name="cloud-outline" size={22} color={colors.accent.primary} />
                </View>
                <View style={styles.subInfo}>
                  <Text style={[styles.subName, { color: colors.text.primary }]}>{item.name || 'Subscription'}</Text>
                  <Text style={[styles.subCycle, { color: colors.text.tertiary }]}>
                    {formatCurrency(Number(item.amount))}{formatBillingCycle(item.billingCycle)}
                  </Text>
                </View>
                <Switch
                  value={item.active}
              ...
              <View style={[styles.subMeta, { borderTopColor: colors.border.subtle }]}>
                <View style={styles.subMetaRow}>
                  ...
                  <Text style={[styles.subMetaText, { color: colors.text.tertiary }]}>
                    Paid by {item.paidBy?.name || 'Unknown'} · {item.memberCount || 1} member{(item.memberCount || 1) > 1 ? 's' : ''}
                  </Text>
                </View>
                {item.nextBillingDate && (
                  <View style={styles.subMetaRow}>
                    ...
                    <Text style={[styles.subMetaText, { color: colors.text.tertiary }]}>
                      Next: {new Date(item.nextBillingDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptyTitle, { color: colors.text.primary }]}>No subscriptions</Text>
            <Text style={[styles.emptyDesc, { color: colors.text.tertiary }]}>Share subscriptions with your group</Text>
          </View>
        }
        ListFooterComponent={<View style={{ height: 100 }} />}
      />

      <TouchableOpacity style={[styles.fab, { backgroundColor: colors.accent.primary }]} onPress={() => navigation.navigate('CreateSharedSubscription', { groupId })}>
...
const styles = StyleSheet.create({
  ...
  upcomingRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, borderBottomWidth: StyleSheet.hairlineWidth, gap: 10 },
  ...
  subIconWrap: { width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  subInfo: { flex: 1 },
  subName: { fontSize: 15, fontWeight: '600', marginBottom: 2 },
  subCycle: { fontSize: 12, fontWeight: '500' },
  subMeta: { flexDirection: 'row', justifyContent: 'space-between', paddingTop: 12, marginTop: 12, borderTopWidth: StyleSheet.hairlineWidth, flexWrap: 'wrap', gap: 6 },
