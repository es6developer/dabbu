  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.bg.primary }]} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={[styles.typeToggle, { backgroundColor: colors.bg.tertiary }]}>
        <TouchableOpacity style={[styles.typeBtn, type === 'expense' && { backgroundColor: colors.status.error }]} onPress={() => setType('expense')}>
          <Text style={[styles.typeBtnText, { color: type === 'expense' ? '#fff' : colors.text.tertiary }]}>Expense</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.typeBtn, type === 'income' && { backgroundColor: colors.status.success }]} onPress={() => setType('income')}>
          <Text style={[styles.typeBtnText, { color: type === 'income' ? '#fff' : colors.text.tertiary }]}>Income</Text>
        </TouchableOpacity>
      </View>

      <Text style={[styles.label, { color: colors.text.tertiary }]}>Amount</Text>
      <View style={[styles.amountRow, { backgroundColor: colors.bg.tertiary, borderColor: colors.border.subtle }]}>
      ...
      <TextInput style={[styles.input, { backgroundColor: colors.bg.tertiary, color: colors.text.primary, borderColor: colors.border.subtle }]} value={description} onChangeText={setDescription} placeholder="What was this for?" placeholderTextColor={colors.text.tertiary} />

      <Text style={[styles.label, { color: colors.text.tertiary }]}>Category</Text>
      <View style={styles.chipRow}>
        {(categories || []).map((cat: any) => (
          <TouchableOpacity key={cat.id} style={[styles.chip, { backgroundColor: colors.bg.tertiary, borderColor: colors.border.subtle }, categoryId === cat.id && { backgroundColor: `${colors.accent.primary}20`, borderColor: colors.accent.primary }]} onPress={() => handleCategorySelect(cat)}>
            <Text style={[styles.chipText, { color: colors.text.tertiary }, categoryId === cat.id && { color: colors.accent.primary, fontWeight: '600' }]}>{cat.name}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <TextInput style={[styles.input, { backgroundColor: colors.bg.tertiary, color: colors.text.primary, borderColor: colors.border.subtle }]} value={tags} onChangeText={setTags} placeholder="e.g. food, grocery" placeholderTextColor={colors.text.tertiary} />

      <View style={styles.switchRow}>
        <Text style={[styles.label, { color: colors.text.tertiary, marginTop: 0 }]}>Recurring Expense</Text>
        <TouchableOpacity style={[styles.recurringToggle, { backgroundColor: colors.bg.tertiary, borderColor: colors.border.subtle }, isRecurring && { backgroundColor: `${colors.accent.primary}20`, borderColor: colors.accent.primary }]} onPress={() => setIsRecurring(!isRecurring)}>
          <Text style={[styles.recurringToggleText, { color: colors.text.tertiary }, isRecurring && { color: colors.accent.primary }]}>{isRecurring ? 'ON' : 'OFF'}</Text>
        </TouchableOpacity>
      </View>
      {isRecurring && (
        <View style={styles.frequencyRow}>
          {['daily', 'weekly', 'monthly', 'yearly'].map((f) => (
            <TouchableOpacity key={f} style={[styles.freqChip, { backgroundColor: colors.bg.tertiary, borderColor: colors.border.subtle }, recurringFrequency === f && { backgroundColor: `${colors.accent.primary}20`, borderColor: colors.accent.primary }]} onPress={() => setRecurringFrequency(f)}>
              <Text style={[styles.freqChipText, { color: colors.text.tertiary }, recurringFrequency === f && { color: colors.accent.primary, fontWeight: '600' }]}>{f.charAt(0).toUpperCase() + f.slice(1)}</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
      ...
      <TouchableOpacity style={[styles.saveBtn, { backgroundColor: colors.accent.primary }, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving}>
        {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>Create Transaction</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 12, fontWeight: '600', marginBottom: 8, marginTop: 16, textTransform: 'uppercase', letterSpacing: 0.8 },
