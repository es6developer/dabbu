...
import {
  ...
  TextInput, Dimensions, Alert, Animated,
} from 'react-native';
...
const GROUP_COLORS: Record<string, string[]> = {
  users: ['#6C5CE7', '#A29BFE'], home: ['#0984E3', '#74B9FF'],
  heart: ['#FF6B6B', '#FF9F9F'], star: ['#FDCB6E', '#FFEAA7'],
  briefcase: ['#00B894', '#55EFC4'], cart: ['#E17055', '#FAB1A0'],
  airplane: ['#4A90D9', '#74B9FF'], restaurant: ['#F7892C', '#F9A44A'],
  car: ['#636E72', '#B2BEC3'], fitness: ['#00CEC9', '#81ECEC'],
};
...
function getGroupColors(icon: string) { return GROUP_COLORS[icon] || ['#F7892C', '#F9A44A']; }
  ...
  if (loading) {
    return (
      <View style={[s.screen, { backgroundColor: colors.bg.primary }]}>
        ...
        {[1, 2, 3].map(i => <SkeletonCard key={i} style={{ marginHorizontal: 16, marginBottom: 12 }} />)}
      </View>
    );
  }
      ...
      <FlatList
          ...
          <Animated.View style={{ opacity: fadeAnim }}>
            <View style={[s.header, { paddingTop: insets.top + 8 }]}>
              ...
              <View style={{ flexDirection: 'row', gap: 8 }}>
                <TouchableOpacity style={[s.addBtn, { backgroundColor: colors.bg.secondary }]} onPress={() => navigation.navigate('Analytics')}>
                ...
                <TouchableOpacity style={s.addBtnPrimary} onPress={handleCreateGroup}>
                  <LinearGradient colors={colors.accent.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.addBtnGrad}>
            ...
            <View style={[s.planBar, { backgroundColor: colors.bg.secondary }]}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <Text style={[s.planText, { color: colors.text.secondary }]}>{groups.length}/{planInfo.maxGroups} circles</Text>
              </View>
              {planInfo.tier === 'free' && (
            ...
            <View style={s.searchRow}>
              <View style={[s.searchBar, { backgroundColor: colors.bg.secondary }]}>
                ...
                <TextInput style={[s.searchInput, { color: colors.text.primary }]} value={search} onChangeText={setSearch} placeholder="Search circles..." placeholderTextColor={colors.text.tertiary} />
            ...
            <View style={s.filterRow}>
              {(['all', 'recent', 'active'] as const).map(option => (
                <TouchableOpacity key={option} style={[s.filterChip, sortBy === option ? s.filterChipActive : { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]} onPress={() => setSortBy(option)}>
                  <Text style={[s.filterText, { color: sortBy === option ? '#FFF' : colors.text.secondary }]}>{option === 'all' ? 'All' : option === 'recent' ? 'Recent' : 'Active'}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </Animated.View>
        ...
        renderItem={({ item }) => {
          ...
          return (
            <TouchableOpacity activeOpacity={0.7} onPress={() => navigation.navigate('GroupExpenses', { groupId: item.id, groupName: item.name })} style={s.cardOuter}>
              <LinearGradient colors={[`${colors2[0]}18`, `${colors2[1]}08`, colors.bg.secondary]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.card}>
                <View style={s.cardTop}>
                  <LinearGradient colors={colors2} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.cardAvatar}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                      <Text style={[s.cardMeta, { color: colors.text.secondary }]}><Ionicons name="people-outline" size={11} color={colors.text.secondary} /> {item._count?.members || 0}</Text>
                      {item.description ? <Text style={[s.cardMeta, { color: colors.text.secondary }]} numberOfLines={1}>{item.description}</Text> : null}
                    </View>
                  </View>
                  <TouchableOpacity style={[s.deleteBtn, { backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)' }]} onPress={() => confirmDelete(item.id)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                    <Ionicons name="trash-outline" size={16} color={colors.text.tertiary} />
                  </TouchableOpacity>
                </View>
                <View style={s.statsRow}>
                  <LinearGradient colors={[`${colors2[0]}15`, 'transparent']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.stat}>
                  ...
                  {budgetLeft !== null ? (
                    <LinearGradient colors={[`${colors2[0]}15`, 'transparent']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.stat}>
                      <Text style={[s.statLabel, { color: colors.text.tertiary }]}>Budget Left</Text>
                    ...
                    <LinearGradient colors={[`${colors2[0]}15`, 'transparent']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.stat}>
                      ...
                      <Text style={[s.statVal, { color: colors.text.primary, fontSize: 12 }]}>{timeAgo(item.createdAt)}</Text>
                    </LinearGradient>
                  )}
                  <LinearGradient colors={[`${colors2[0]}15`, 'transparent']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.stat}>
                ...
                {ed.latest && (
                  <View style={[s.latest, { backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)' }]}>
        ...
        ListEmptyComponent={
          <View style={s.empty}>
            ...
            <Text style={[s.emptyTitle, { color: colors.text.primary }]}>{search ? 'No circles found' : 'No circles yet'}</Text>
            <Text style={[s.emptyDesc, { color: colors.text.tertiary }]}>{search ? 'Try a different search term' : 'Create your first expense circle'}</Text>
            {!search && (
...
const s = StyleSheet.create({
  ...
  cardAvatar: { width: 48, height: 48, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
  ...
  cardFooter: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 4, borderTopWidth: 1, paddingTop: 12 },
