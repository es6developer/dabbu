...
export function ReminderDetailScreen() {
  ...
  if (!reminder) return <View style={[styles.loading, { backgroundColor: colors.bg.primary }]}><Text style={[styles.errorText, { color: colors.status.error }]}>Reminder not found</Text></View>;
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.bg.primary }]}>
      <View style={[styles.header, { backgroundColor: colors.bg.secondary }]}>
        <View style={styles.badgeRow}>
          <View style={[styles.badge, { backgroundColor: `${getPriorityColor(reminder.priority)}22` }]}>
            <Text style={[styles.badgeText, { color: getPriorityColor(reminder.priority) }]}>{reminder.priority?.toUpperCase()}</Text>
          </View>
          <View style={[styles.badge, { backgroundColor: `${colors.accent.primary}18` }]}>
            <Text style={[styles.badgeText, { color: colors.accent.primary }]}>{reminder.type?.toUpperCase()}</Text>
          </View>
          {reminder.status && (
            <View style={[styles.badge, { backgroundColor: reminder.status === 'completed' ? `${colors.status.success}18` : `${colors.status.warning}18` }]}>
              <Text style={[styles.badgeText, { color: reminder.status === 'completed' ? colors.status.success : colors.status.warning }]}>{reminder.status.toUpperCase()}</Text>
            </View>
          )}
        </View>
      ...
      <View style={styles.section}>
        {reminder.description ? <Text style={[styles.description, { color: colors.text.secondary }]}>{reminder.description}</Text> : null}
        ...
        <View style={[styles.detailRow, { borderBottomColor: colors.border.subtle }]}>
          ...
          <Text style={[styles.detailValue, { color: colors.text.primary }]}>{new Date(reminder.startDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</Text>
        </View>
        {reminder.dueDate && (
          <View style={[styles.detailRow, { borderBottomColor: colors.border.subtle }]}>
            <Text style={[styles.detailLabel, { color: colors.text.tertiary }]}>Due Date</Text>
            <Text style={[styles.detailValue, { color: colors.text.primary }]}>{new Date(reminder.dueDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</Text>
          </View>
        )}
        {reminder.category && (
          <View style={[styles.detailRow, { borderBottomColor: colors.border.subtle }]}>
            <Text style={[styles.detailLabel, { color: colors.text.tertiary }]}>Category</Text>
            <Text style={[styles.detailValue, { color: colors.text.primary }]}>{reminder.category}</Text>
          </View>
        )}
        {reminder.isRecurring && (
          <View style={[styles.detailRow, { borderBottomColor: colors.border.subtle }]}>
            <Text style={[styles.detailLabel, { color: colors.text.tertiary }]}>Recurring</Text>
            <Text style={[styles.detailValue, { color: colors.text.primary }]}>{reminder.frequency} (every {reminder.interval || 1})</Text>
          </View>
        )}
        <View style={[styles.detailRow, { borderBottomColor: colors.border.subtle }]}>
          <Text style={[styles.detailLabel, { color: colors.text.tertiary }]}>Created</Text>
          <Text style={[styles.detailValue, { color: colors.text.primary }]}>{new Date(reminder.createdAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</Text>
        </View>
      </View>

      <View style={styles.actions}>
        <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.status.success }]} onPress={handleComplete} disabled={actionLoading}>
          <Text style={styles.actionBtnText}>✓ Complete</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.accent.primary }]} onPress={() => setSnoozeVisible(true)} disabled={actionLoading}>
          <Text style={styles.actionBtnText}>⏰ Snooze</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]} onPress={() => navigation.navigate('CreateReminder', { reminder })} disabled={actionLoading}>
          <Text style={[styles.actionBtnText, { color: colors.text.primary }]}>✎ Edit</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.actionBtn, { backgroundColor: `${colors.status.error}18`, borderColor: `${colors.status.error}40` }]} onPress={handleDelete} disabled={actionLoading}>
        ...
        <View style={[styles.modalOverlay, { backgroundColor: 'rgba(0,0,0,0.6)' }]}>
          <View style={[styles.modalContent, { backgroundColor: colors.bg.secondary }]}>
            ...
            {SNOOZE_OPTIONS.map((opt) => (
              <TouchableOpacity key={opt.value} style={[styles.snoozeOption, { borderBottomColor: colors.border.subtle }]} onPress={() => handleSnooze(opt.value)}>
                <Text style={[styles.snoozeOptionText, { color: colors.text.primary }]}>{opt.label}</Text>
              </TouchableOpacity>
            ))}
...
const styles = StyleSheet.create({
  ...
  actionBtn: { paddingVertical: 14, paddingHorizontal: 20, borderRadius: 14, alignItems: 'center', flex: 1, minWidth: '45%', borderWidth: 1, borderColor: 'transparent' },
