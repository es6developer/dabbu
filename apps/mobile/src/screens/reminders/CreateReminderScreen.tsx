...
const DAYS_OF_WEEK = [{ label: 'Sun', value: 0 }, { label: 'Mon', value: 1 }, { label: 'Tue', value: 2 }, { label: 'Wed', value: 3 }, { label: 'Thu', value: 4 }, { label: 'Fri', value: 5 }, { label: 'Sat', value: 6 }];
...
export function CreateReminderScreen() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (isRecurring) {
        ...
        data.interval = parseInt(interval, 10) || 1;
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.bg.primary }]} contentContainerStyle={[styles.content, { paddingBottom: insets.bottom + 100 }]} keyboardShouldPersistTaps="handled">
      ...
      <TextInput style={[styles.input, { backgroundColor: colors.bg.secondary, color: colors.text.primary, borderColor: colors.border.subtle }]} value={title} onChangeText={setTitle} placeholder="Reminder title" placeholderTextColor={colors.text.tertiary} />
      ...
      <TouchableOpacity style={[styles.picker, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]} onPress={() => setShowTypePicker(!showTypePicker)}>
        <Text style={[styles.pickerText, { color: colors.text.primary }]}>{type.charAt(0).toUpperCase() + type.slice(1)}</Text>
      ...
      {showTypePicker && (
        <View style={[styles.pickerOptions, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]}>
          {REMINDER_TYPES.map((t) => (
            <TouchableOpacity key={t} style={[styles.pickerOption, { borderBottomColor: colors.border.subtle }, type === t && { backgroundColor: `${colors.accent.primary}18` }]} onPress={() => { setType(t); setShowTypePicker(false); }}>
              <Text style={[styles.pickerOptionText, { color: colors.text.primary }, type === t && { color: colors.accent.primary }]}>{t.charAt(0).toUpperCase() + t.slice(1)}</Text>
            </TouchableOpacity>
          ))}
        </View>
      )}
      ...
      <TextInput style={[styles.input, { backgroundColor: colors.bg.secondary, color: colors.text.primary, borderColor: colors.border.subtle }]} value={category} onChangeText={setCategory} placeholder="e.g. Work, Personal" placeholderTextColor={colors.text.tertiary} />

      <View style={styles.switchRow}>
        <Text style={[styles.label, { color: colors.text.secondary }]}>Recurring</Text>
        <Switch value={isRecurring} onValueChange={setIsRecurring} trackColor={{ false: colors.border.subtle, true: colors.accent.primary }} thumbColor={isRecurring ? '#FFFFFF' : colors.text.tertiary} />
      </View>

      {isRecurring && (
        <>
          <Text style={[styles.label, { color: colors.text.secondary }]}>Frequency</Text>
          <TouchableOpacity style={[styles.picker, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]} onPress={() => setShowFreqPicker(!showFreqPicker)}>
            <Text style={[styles.pickerText, { color: colors.text.primary }]}>{frequency.charAt(0).toUpperCase() + frequency.slice(1)}</Text>
            <Text style={[styles.pickerArrow, { color: colors.text.tertiary }]}>▼</Text>
          </TouchableOpacity>
          {showFreqPicker && (
            <View style={[styles.pickerOptions, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }]}>
              {FREQUENCIES.map((f) => (
                <TouchableOpacity key={f} style={[styles.pickerOption, { borderBottomColor: colors.border.subtle }, frequency === f && { backgroundColor: `${colors.accent.primary}18` }]} onPress={() => { setFrequency(f); setShowFreqPicker(false); }}>
                  <Text style={[styles.pickerOptionText, { color: colors.text.primary }, frequency === f && { color: colors.accent.primary }]}>{f.charAt(0).toUpperCase() + f.slice(1)}</Text>
                </TouchableOpacity>
              ))}
            </View>
          )}
          ...
          <TextInput style={[styles.input, { backgroundColor: colors.bg.secondary, color: colors.text.primary, borderColor: colors.border.subtle }]} value={interval} onChangeText={setInterval} placeholder="1" placeholderTextColor={colors.text.tertiary} keyboardType="number-pad" />

          <Text style={[styles.label, { color: colors.text.secondary }]}>Day of Week</Text>
          <View style={styles.dayRow}>
            {DAYS_OF_WEEK.map((d) => (
              <TouchableOpacity key={d.value} style={[styles.dayBtn, { backgroundColor: colors.bg.secondary, borderColor: colors.border.subtle }, dayOfWeek === d.value && { backgroundColor: colors.accent.primary, borderColor: colors.accent.primary }]} onPress={() => setDayOfWeek(d.value)}>
                <Text style={[styles.dayBtnText, { color: colors.text.tertiary }, dayOfWeek === d.value && { color: '#FFFFFF' }]}>{d.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </>
      ...
      <TouchableOpacity style={[styles.saveBtn, { backgroundColor: colors.accent.primary }, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving}>
        {saving ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.saveBtnText}>Create Reminder</Text>}
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  dayBtn: { width: 40, height: 40, borderRadius: 22, justifyContent: 'center', alignItems: 'center', borderWidth: 1 },
