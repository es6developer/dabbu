import { ConversationState, ConversationStep, IntentType, AIResponse } from '../types';
...
export function cancelResponse(): AIResponse {
  return { action: 'cancel', message: 'Cancelled. What else can I help with?' };
}

export function askResponse(question: string, field: string, options: string[], context?: Record<string, any>): AIResponse {
  return { action: 'ask', message: question, field, options, data: context };
}

export function messageResponse(text: string, data?: any): AIResponse {
  return { action: 'message', message: text, data };
}
