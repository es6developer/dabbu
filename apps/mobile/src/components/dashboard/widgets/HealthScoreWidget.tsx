...
const subLabels = ['Savings', 'Debt', 'Budget', 'Emergency Fund', 'Goal Progress'];
...
export function HealthScoreWidget({ data }: { data: any }) {
  ...
  const subs = healthScore?.subScores || [];
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={[styles.title, { color: colors.text.primary }]}>Financial Health</Text>
      </View>
            ...
            return (
              <View key={label} style={styles.subRow}>
                ...
                <View style={[styles.subBarBg, { backgroundColor: colors.bg.tertiary }]}>
                  <View style={[styles.subBarFill, { width: `${Math.min(val, 100)}%`, backgroundColor: getScoreColor(val) }]} />
                </View>
              </View>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  subLabel: { fontSize: 10, fontWeight: '500', width: 80 },
  subBarBg: { flex: 1, height: 5, borderRadius: 8.5, overflow: 'hidden' },
  subBarFill: { height: '100%', borderRadius: 8.5 },
});
