...
export function CoupleHealthWidget({ data }: { data: any }) {
  const { colors } = useTheme();
  const { coupleHealth } = data || {};
  const score = Math.min(100, Math.max(0, Number(coupleHealth?.score ?? 0)));
  const subs = coupleHealth?.subScores || [];
...
const styles = StyleSheet.create({
  ...
  subLabel: { fontSize: 10, fontWeight: '500', width: 90 },
