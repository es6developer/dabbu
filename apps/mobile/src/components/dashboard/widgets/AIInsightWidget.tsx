...
export function AIInsightWidget({ data }: { data: any }) {
  const { colors } = useTheme();
  const { aiInsight } = data || {};

  if (!aiInsight?.text && !aiInsight?.title) {
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <AntDesign name="bulb1" size={18} color={colors.accent.primary}  />
          <Text style={[styles.title, { color: colors.text.primary }]}>AI Insight</Text>
        </View>
  ...
  return (
    <View style={styles.container}>
      ...
      {aiInsight.title ? <Text style={[styles.insightTitle, { color: colors.text.primary }]}>{aiInsight.title}</Text> : null}
      {aiInsight.text ? <Text style={[styles.text, { color: colors.text.secondary }]}>{aiInsight.text}</Text> : null}
    </View>
  );
}
