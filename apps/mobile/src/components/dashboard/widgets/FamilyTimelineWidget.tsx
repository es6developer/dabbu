...
export function FamilyTimelineWidget({ data }: { data: any }) {
  const { colors } = useTheme();
  const { familyTimeline } = data || {};
  const events = Array.isArray(familyTimeline) ? familyTimeline : [];

  if (!events.length) {
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <AntDesign name="clockcircleo" size={18} color={colors.accent.primary}  />
          <Text style={[styles.title, { color: colors.text.primary }]}>Family Timeline</Text>
        </View>
