...
export function SkeletonCard({ style }: { style?: any }) {
  return <Skeleton height={120} borderRadius={28} style={style} />;
}
      ...
      {Array.from({ length: count }, (_, i) => (
        <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: spacingTokens.md }}>
          ...
          <View style={{ flex: 1, gap: 6 }}>
            <Skeleton width={i % 3 === 0 ? '80%' : '60%'} height={14} borderRadius={12} />
            <Skeleton width={i % 2 === 0 ? '50%' : '40%'} height={11} borderRadius={10} />
          </View>
  ...
  return (
    <View style={{ paddingTop: spacingTokens.lg, gap: spacingTokens.lg }}>
      <View style={{ paddingHorizontal: spacingTokens.xl, flexDirection: 'row', alignItems: 'center', gap: spacingTokens.md }}>
        ...
        <Skeleton width="60%" height={22} borderRadius={16} />
      </View>
      <View style={{ paddingHorizontal: spacingTokens.xl }}>
        <Skeleton width="100%" height={140} borderRadius={28} />
      </View>
      ...
      <View style={{ gap: spacingTokens.md }}>
        {[1, 2, 3, 4].map((i) => (
          <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: spacingTokens.sm }}>
            <Skeleton width={12} height={12} borderRadius={12} />
            <Skeleton width="60%" height={14} borderRadius={12} />
            <View style={{ flex: 1 }} />
            <Skeleton width={50} height={14} borderRadius={12} />
          </View>
        ))}
      </View>
    </View>
    ...
    <View style={{ paddingHorizontal: spacingTokens.xl, paddingTop: spacingTokens.lg, gap: spacingTokens.xl }}>
      {[1, 2, 3, 4].map((i) => (
        <View key={i} style={{ gap: spacingTokens.sm }}>
          <Skeleton width={80} height={12} borderRadius={12} />
          <Skeleton width="100%" height={48} borderRadius={20} />
        </View>
      ))}
      <Skeleton width="100%" height={52} borderRadius={20} style={{ marginTop: spacingTokens.sm }} />
    </View>
  );
}
    ...
    <View style={{ paddingHorizontal: spacingTokens.xl, paddingTop: spacingTokens.lg, gap: spacingTokens.lg }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <View style={{ gap: spacingTokens.xs }}>
          <Skeleton width={120} height={12} borderRadius={12} />
          <Skeleton width={180} height={22} borderRadius={16} />
        </View>
        <Skeleton width={44} height={44} borderRadius={30} />
      </View>
