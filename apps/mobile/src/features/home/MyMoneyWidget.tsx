...
export function MyMoneyWidget({ income, expense }: { income: number; expense: number }) {
  const { colors } = useTheme();
  const savings = income - expense;
  return (
    <View style={{ paddingHorizontal: 20, marginBottom: 8 }}>
      <Text style={{ fontSize: 13, fontWeight: '700', color: colors.text.secondary, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.5 }}>My Money</Text>
