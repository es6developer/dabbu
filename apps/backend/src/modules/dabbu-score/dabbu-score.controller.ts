...
export class DabbuScoreController {
  ...
  @Get()
  @ApiOperation({ summary: 'Get dabbu score for the current user or a specific entity' })
  @ApiQuery({ name: 'entityType', enum: ['USER', 'SPACE'], required: false })
  ...
  async getScore(
    ...
    return { data: await this.service.getScore(type, id) };
  }
  ...
  @Get('components')
  @ApiOperation({ summary: 'Get component breakdown for the current user or a specific entity' })
  @ApiQuery({ name: 'entityType', enum: ['USER', 'SPACE'], required: false })
  @ApiQuery({ name: 'entityId', type: String, required: false })
  async getComponents(
    @CurrentUser('id') userId: string,
    ...
    const type = entityType || 'USER';
    const id = entityId || userId;
    return { data: await this.service.getComponents(type, id) };
  }
