...
export interface PartneredDashboardData {
  ...
  spendingComparison: {
    you: { category: string; amount: number }[];
    partner: { category: string; amount: number }[];
  };
}
...
export class LensDashboardService {
  ...
  async getPartneredDashboard(userId: string): Promise<PartneredDashboardData | null> {
    const coupleSpaces = await this.prisma.space
      .findMany({
  ...
  async getFamilyDashboard(userId: string): Promise<FamilyDashboardData | null> {
    const families = await this.prisma.familyMember
      .findMany({
        where: { userId },
    ...
    const familyMembers = await this.prisma.familyMember
      .count({
    ...
    return {
      ...
      familyGoals: goals.map((g) => ({
        ...
        name: g.name,
        targetAmount: Number(g.targetAmount),
        currentAmount: Number(g.savedAmount),
        progress:
          Number(g.targetAmount) > 0
  ...
  async getFullDashboard(userId: string): Promise<FullDashboardData | null> {
    ...
    const spaces = await this.prisma.space
      .findMany({
    ...
    const investments = await this.prisma.investment
      .findMany({
    ...
    const spaceCounts = await Promise.all(
      spaces.map((s) => this.prisma.spaceMember.count({ where: { spaceId: s.id } }).catch(() => 0)),
    );
    ...
    return {
      ...
      partnered: partnered || {
        ...
        sharedGoals: [],
        sharedBills: [],
        settlementStatus: { totalOwed: 0, youOwe: 0, youAreOwed: 0 },
        spendingComparison: { you: [], partner: [] },
      },
      family: family || {
        familyIncome: 0,
