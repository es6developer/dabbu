...
export class CoupleDashboardService {
  ...
  async getDashboard(userId: string) {
    ...
    const userTotal = Number(userTxns._sum.amount || 0);
    const partnerTotal = Number(partnerTxns._sum.amount || 0);
  ...
  async getHealthScore(userId: string) {
    ...
    const { groupId, partnerId } = coupleInfo;
    const [userScore, partnerScore, coupleLevel] = await Promise.all([
      this.prisma.aiScore.findFirst({ where: { userId }, orderBy: { createdAt: 'desc' } }),
      this.prisma.aiScore.findFirst({
        where: { userId: partnerId },
        orderBy: { createdAt: 'desc' },
    ...
    return {
      overallScore: coupleLevel?.healthScore || 0,
      level: coupleLevel?.level || 1,
      xp: coupleLevel?.xp || 0,
      members: [
        {
          userId,
          overallScore: userScore?.overallScore || 0,
          financialLevel: userScore?.financialLevel,
        },
        {
          userId: partnerId,
          overallScore: partnerScore?.overallScore || 0,
