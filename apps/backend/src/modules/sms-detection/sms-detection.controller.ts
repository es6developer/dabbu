...
export class SmsDetectionController {
  ...
  async parseSms(@CurrentUser('id') userId: string, @Body() body: { message: string; sender: string }) {
  ...
  async detectAndCreate(@CurrentUser('id') userId: string, @Body() body: { message: string; sender: string }) {
