...
const CATEGORY_KEYWORDS: { keywords: string[]; category: string }[] = [
  {
    keywords: [
      'restaurant', 'hotel', 'cafe', 'food', 'dining', 'pizza', 'burger',
      'lunch', 'dinner', 'breakfast', 'tiffin', 'biryani', 'curry', 'snack',
    ],
    category: 'Food & Dining',
  },
  {
    keywords: [
      'grocery', 'groceries', 'supermarket', 'mart', 'provisions',
      'vegetables', 'fruits', 'milk', 'dairy', 'bakery', 'general store', 'kirana',
    ],
    category: 'Groceries',
  },
  {
    keywords: [
      'petrol', 'diesel', 'fuel', 'service station', 'indian oil',
      'hp', 'bharat petroleum', 'shell', 'parking', 'toll',
    ],
    category: 'Transportation',
  },
  {
    keywords: ['electronics', 'clothing', 'apparel', 'footwear', 'mall', 'retail', 'superstore', 'departmental'],
    category: 'Shopping',
  },
  {
    keywords: ['electricity', 'water bill', 'gas bill', 'broadband', 'internet', 'telephone', 'recharge', 'utility'],
    category: 'Bills & Utilities',
  },
  { keywords: ['movie', 'cinema', 'netflix', 'game', 'entertainment', 'amusement', 'ticket'], category: 'Entertainment' },
  {
    keywords: [
      'hospital', 'doctor', 'clinic', 'pharmacy', 'medicine', 'medical', 'health',
      'diagnostic', 'dentist', 'eye', 'consultation', 'fees', 'checkup',
      'lab', 'test', 'scan', 'xray', 'mri', 'ecg', 'blood', 'prescription',
      'surgery', 'pathology', 'vaccine', 'health check',
    ],
    category: 'Healthcare',
  },
  {
    keywords: ['school', 'college', 'university', 'tuition', 'course', 'book', 'stationery', 'training'],
    category: 'Education',
  },
...
export class BillScannerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    // lazy init - workers created on first scan
  }
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('OCR timed out')), 25_000),
        ),
      ]);
      return result;
    } catch (err: any) {
      ...
      return {
        amount: 0, merchant: '', date: '', description: '', category: 'Other',
        items: [], confidence: 0, rawText: err.message,
      };
    }
  }
  ...
  private async runFastOcr(imageBuffer: Buffer): Promise<BillScanResult> {
    ...
    const text = await this.runTesseract(enhanced);
    const cleaned = this.normalizeOcrText(text || 'NO TEXT EXTRACTED');
  ...
  private async enhanceImage(imageBuffer: Buffer): Promise<Buffer> {
    try {
      ...
      const targetWidth = Math.min(Math.max(origWidth, 2000), 3000);
      ...
      this.logger.log(`Enhanced image ${origWidth}->${targetWidth}px (${Math.round(enhanced.length / 1024)} KB)`);
      return enhanced;
    } catch (err) {
      this.logger.warn('Image enhancement failed, using original');
      return imageBuffer;
    }
  }
  ...
  private async runTesseract(imageBuffer: Buffer): Promise<string> {
    try {
      ...
      this.logger.log(`Tesseract extracted ${text.length} chars (conf: ${Math.round(data.confidence || 0)})`);
      return text;
    } catch (err: any) {
      this.logger.error(`Tesseract failed: ${err.message}`);
      return '';
    }
  }
  ...
  private async getTesseractWorker(): Promise<any> {
    if (this.tesseractWorker) {
      try {
        ...
        this.logger.warn('Worker died, creating new one');
      }
    }
  ...
  async checkHealth(): Promise<{
    ok: boolean; workerOk: boolean; sharpOk: boolean; preloaded: boolean; message: string;
  }> {
    const health = { ok: false, workerOk: false, sharpOk: false, preloaded: false, message: '' };
    try {
      await sharp(
    ...
    try {
      const worker = await createWorker('eng', OEM.LSTM_ONLY, {
        logger: () => {}, cachePath: '/tmp', cacheMethod: 'readWrite',
      });
      ...
      health.message = `tesseract init failed: ${e.message}`;
    }
    return health;
  }
  ...
  private parseBillText(text: string): BillScanResult {
    ...
    return { amount, merchant, date, description: description || merchant, category, items, confidence, rawText: text };
  }

  private extractMerchant(lines: string[]): string {
    const firstWordSkip = new Set([
      'gstin', 'gst', 'invoice', 'bill', 'receipt', 'tax', 'sale', 'cash',
      'total', 'amount', 'date', 'phone', 'mobile', 'tel', 'www', 'http',
      'email', 'address', 'item', 'qty', 'rate', 'price', 'hsn', 'sac',
  ...
  private extractAmount(text: string, lines: string[]): number {
    ...
    for (const label of totalLabels) {
      const regex = new RegExp(
        `${label.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*(?:[:=\\-]|rs\\.?|inr|₹)?\\s*(?:rs\\.?\\s*|inr\\s*|₹\\s*)?([\\d,]+(?:\\.\\d{1,2})?)`, 'i',
      );
      const match = text.match(regex);
      if (match) {
        const val = parseFloat(match[1].replace(/,/g, ''));
        if (!isNaN(val) && val > 0 && val < 9999999 && !this.looksLikePhone(val)) {
          const priority = /grand|net|payable|due|paid|bill|total amount|amount payable/i.test(label) ? 18
            : /subtotal|sub total/i.test(label) ? 7 : 14;
    ...
    if (items.length > 0) {
      ...
      candidates.push({ value: Math.round(sumFromItems * 100) / 100, priority: 6, source: 'items' });
    }
  ...
  private extractItems(lines: string[]): { name: string; price: number; quantity?: number }[] {
    ...
    const skipLineSet = new Set([
      'gstin', 'gst', 'invoice', 'bill', 'receipt', 'tax', 'total', 'amount', 'cash',
      'change', 'phone', 'mobile', 'tel', 'website', 'email', 'address', 'thank',
      'have a nice day', 'visit again', 'saved', 'card', 'credit', 'debit', 'upi',
      'payment', 'change due', 'round off', 'subtotal', 'item', 'qty', 'rate', 'price',
      'description', 'sub total', 'net amount', 'grand total', 'date', 'bill no',
      'invoice no', 'hsn', 'sac', 'mrp', 'cgst', 'sgst', 'igst', 'cess', 'discount',
      'savings', 'round up', 'round down', 'paid by', 'pay by', 'cashier', 'counter',
    ]);
