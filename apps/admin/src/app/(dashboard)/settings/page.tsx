  ...
  async function handleSendTest() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message?.[0] || err.message || 'Failed');
      }
              ...
              <div className="glass-panel p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <SelectField
                    ...
                    onChange={(v) => updateField('newUserDefaultPlan', v)}
                    options={[
                  ...
                  <TextField
                    ...
                    value={String(config.trialDurationDays)}
                    onChange={(v) => updateField('trialDurationDays', parseInt(v))}
                    type="number"
                  />
                </div>
              </div>
              ...
              <div className="glass-panel p-6">
                ...
                <div className="space-y-4">
                  <ToggleRow
                    ...
                    checked={!!config.enableSignups}
                    onChange={(v) => updateField('enableSignups', v)}
                  />
                  <ToggleRow
                    label="Enable API Access"
                    checked={!!config.enableApiAccess}
                    onChange={(v) => updateField('enableApiAccess', v)}
                  />
                </div>
              </div>
              ...
              <div className="glass-panel p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <TextField
                    ...
                    onChange={(v) => updateField('maxLoginAttempts', parseInt(v))}
                    type="number"
                  />
                  <TextField
                    label="Session Timeout (min)"
                    value={String(config.sessionTimeout)}
                    onChange={(v) => updateField('sessionTimeout', parseInt(v))}
                    type="number"
                  />
                </div>
                <div className="mt-4">
                  <ToggleRow
                    ...
                    onChange={(v) => updateField('otpEnabled', v)}
                  />
                </div>
              </div>
                ...
                {mfaRequired && mfaVerified && !mfaSecret ? (
                  ...
                  <div className="space-y-4">
                    <p className="text-sm text-white/60">
                      ...
                      Authy) or enter the setup key manually.
                    </p>
                    <div className="flex justify-center">
                      <div className="bg-white p-4 rounded-xl">
                        {mfaQrUrl ? (
                          <img src={mfaQrUrl} alt="MFA QR Code" className="w-40 h-40" />
                        ) : (
                    ...
                    <div className="text-center">
                      ...
                      <code className="text-sm font-mono bg-white/[0.06] px-3 py-1.5 rounded-lg text-indigo-300 select-all">
                        {mfaSecret}
                      </code>
                    </div>
                    <div className="pt-2">
                      ...
                      <div className="flex gap-3">
                        <input
                          ...
                          value={mfaSetupCode}
                          onChange={(e) =>
                            setMfaSetupCode(e.target.value.replace(/\D/g, '').slice(0, 6))
                          }
          ...
          {activeTab === 'email' && (
            <div className="space-y-6">
              <div className="glass-panel p-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <TextField
                    ...
                    onChange={(v) => updateField('smtpHost', v || null)}
                  />
                  <TextField
                    label="SMTP Port"
                    value={config.smtpPort ? String(config.smtpPort) : ''}
                    onChange={(v) => updateField('smtpPort', v ? parseInt(v) : null)}
                    type="number"
                  />
                  <TextField
                    label="SMTP User"
                    value={config.smtpUser || ''}
                    onChange={(v) => updateField('smtpUser', v || null)}
                  />
                  <TextField
                    label="SMTP Password"
                    value={config.smtpPass || ''}
                    onChange={(v) => updateField('smtpPass', v || null)}
                    type="password"
                  />
                  <TextField
                    label="From Name"
                    value={config.smtpFromName || ''}
                    onChange={(v) => updateField('smtpFromName', v || null)}
                  />
                  <TextField
                    label="From Email"
                    value={config.smtpFromEmail || ''}
                    onChange={(v) => updateField('smtpFromEmail', v || null)}
                  />
                </div>
                <div className="mt-4">
                  <ToggleRow
                    label="Use SSL/TLS (secure port)"
              ...
              <div className="glass-panel p-6">
                ...
                {testResult && (
                  <div
                    className={`flex items-center gap-2 text-sm mt-3 px-3 py-2 rounded-xl ${testResult.ok ? 'text-emerald-400 bg-emerald-500/10 border border-emerald-500/20' : 'text-red-400 bg-red-500/10 border border-red-500/20'}`}
                  >
                    {testResult.ok ? (
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setDataRetention({ retentionPeriods: { error: 'Failed to load' } });
                        } finally {
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                    <div>
                      ...
                      <p className="text-xs text-white/40 mt-1">
                        Purge expired analytics, sessions, logs, and exports per policy
                      </p>
                    </div>
                    <button
                      onClick={async () => {
                      ...
                      className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-xl bg-amber-500/20 text-amber-300 border border-amber-500/20 hover:bg-amber-500/30 disabled:opacity-50 transition-all"
                    >
                      {enforceLoading ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
                  ...
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                    <div>
                      ...
                      <p className="text-xs text-white/40 mt-1">
                        Remove users with no activity in the last 365 days
                      </p>
                    </div>
                    <button
                      onClick={async () => {
                        if (
                          !confirm(
                            'Are you sure you want to delete all users inactive for 365+ days?',
                          )
                        ) {
                          return;
                        }
                  ...
                  <div className="flex items-center justify-between p-4 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                    <div>
                      ...
                      <p className="text-xs text-white/40 mt-1">
                        Download a JSON file with all registered users and their metadata
                      </p>
                    </div>
                    <button
                      onClick={async () => {
                ...
                <div className="glass-panel p-6">
                  ...
                  <div className="space-y-2 text-sm">
                    <p className="text-white/60">
                      <span className="text-white/80 font-medium">GDPR Contact:</span>{' '}
                      {dataRetention.gdprContact}
                    </p>
                    <p className="text-white/60">
                      <span className="text-white/80 font-medium">DPO:</span>{' '}
                      {dataRetention.dataProtectionOfficer}
                    </p>
                    <p className="text-white/60">
                      <span className="text-white/80 font-medium">Legal Basis:</span>{' '}
                      {dataRetention.legalBase}
                    </p>
                    <p className="text-white/60">
                      <span className="text-white/80 font-medium">Deletion Process:</span>{' '}
                      {dataRetention.deletionProcess}
                    </p>
                    <p className="text-white/60">
                      <span className="text-white/80 font-medium">Export Formats:</span>{' '}
                      {dataRetention.exportFormats?.join(', ')}
                    </p>
                  </div>
                </div>
              )}
...
function ToggleRow({
  ...
  return (
    <label className="flex items-center justify-between py-2">
      ...
      >
        <span
          className={`inline-block h-5 w-5 rounded-full bg-white shadow-sm transform transition-transform mt-0.5 ml-0.5 ${checked ? 'translate-x-5' : ''}`}
        />
      </button>
    </label>
  );
