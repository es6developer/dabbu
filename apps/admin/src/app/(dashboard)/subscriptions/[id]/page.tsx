...
import { ChevronLeft, CreditCard, User, Calendar, Loader2 } from 'lucide-react';
...
export default function SubscriptionDetailPage() {
  ...
  function statusColor(status: string) {
    switch (status) {
      ...
      case 'expired':
        return 'bg-white/[0.04] text-white/40 border border-white/[0.06]';
      default:
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {sub.trialEnd && (
            <div>
              ...
              <span className="text-white font-medium">
                {new Date(sub.trialEnd).toLocaleString()}
              </span>
            </div>
          )}
          {sub.cancelledAt && (
            <div>
              ...
              <span className="text-white font-medium">
                {new Date(sub.cancelledAt).toLocaleString()}
              </span>
            </div>
          )}
        </div>
